...
// --- שדרוג וטיוב מסד הנתונים ---
app.get('/setup-db', async (req, res) => {
  try {
    // יצירת טבלאות (עם IF NOT EXISTS למניעת שגיאות)
    ...
    // הוספת עמודות חסרות (למקרה שהטבלה קיימת בגרסה ישנה)
    try {
        await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS age_group VARCHAR(20) DEFAULT 'adult'");
        await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS weekly_allowance DECIMAL(10, 2) DEFAULT 0");
        await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS interest_rate DECIMAL(5, 2) DEFAULT 0");
        await client.query("ALTER TABLE users ADD COLUMN IF NOT EXISTS xp INTEGER DEFAULT 0");
    } catch(e) { console.log('Columns exist or error:', e.message); }
    ...
    res.send(`<h2 style="color: green;">Database & Tables Verified! Ready to rock. 🚀</h2>`);
  } catch (err) { res.status(500).send(`Error: ${err.message}`); }
});

// --- API Endpoints ---

// יצירת יעד (תיקון: וידוא המרה למספרים)
...
// יום התשלום (PayDay) - מאובטח!
app.post('/api/bank/payday', async (req, res) => {
    const { userId } = req.body; // חייב לקבל את ה-ID של המבקש
    try {
        // 1. בדיקת אבטחה: האם המבקש הוא הורה?
        const requesterRes = await client.query('SELECT role FROM users WHERE id = $1', [userId]);
        if (requesterRes.rows.length === 0 || requesterRes.rows[0].role !== 'parent') {
            return res.status(403).json({ success: false, message: 'גישה נדחתה: רק הורים יכולים להפעיל יום תשלום' });
        }
        ...
        for (const child of children) {
            ...
            if (child.weekly_allowance > 0) {
                ...
                await client.query(`INSERT INTO transactions (user_id, amount, description, category, type) VALUES ($1, $2, 'דמי כיס', 'income', 'income')`, [child.id, child.weekly_allowance]);
                report.push(`${child.name}: +₪${child.weekly_allowance} (דמי כיס)`);
            }
            // ריבית
            if (child.interest_rate > 0 && child.balance > 0) {
                const interest = (parseFloat(child.balance) * parseFloat(child.interest_rate)) / 100;
                if (interest > 0) {
                    await client.query(`UPDATE users SET balance = balance + $1 WHERE id = $2`, [interest, child.id]);
                    await client.query(`INSERT INTO transactions (user_id, amount, description, category, type) VALUES ($1, $2, 'ריבית שבועית', 'savings', 'income')`, [child.id, interest]);
                    report.push(`${child.name}: +₪${interest.toFixed(2)} (ריבית ${child.interest_rate}%)`);
                }
            }
        }
...
app.get('/api/data/:userId', async (req, res) => {
  ...
  try {
    ...
    let familyMembers = [];
    if (user.role === 'parent') { familyMembers = (await client.query('SELECT id, name, balance, role, age_group, weekly_allowance, interest_rate FROM users ORDER BY id')).rows; }
    let transQuery = `SELECT t.*, u.name as user_name FROM transactions t LEFT JOIN users u ON t.user_id = u.id`; if (user.role === 'child') transQuery += ` WHERE t.user_id = ${userId}`; transQuery += ` ORDER BY t.date DESC LIMIT 20`;
    const transRes = await client.query(transQuery);
    let tasksQuery = `SELECT t.*, u.name as assignee_name FROM tasks t LEFT JOIN users u ON t.assigned_to = u.id `; if (user.role === 'child') tasksQuery += ` WHERE t.assigned_to = ${userId} AND t.status != 'approved'`; else tasksQuery += ` WHERE t.status != 'approved'`; tasksQuery += ` ORDER BY t.id DESC`;
    ...
    let loansQuery = `SELECT l.*, u.name as user_name FROM loans l LEFT JOIN users u ON l.user_id = u.id`; if (user.role === 'child') loansQuery += ` WHERE l.user_id = ${userId}`; else loansQuery += ` WHERE l.status != 'paid'`; loansQuery += ` ORDER BY l.created_at DESC`;
    const loansRes = await client.query(loansQuery);
    res.json({ user, transactions: transRes.rows, family: familyMembers, tasks: tasksRes.rows, shopping_list: shopRes.rows, goals: goalsRes.rows, loans: loansRes.rows });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
