...
const seedQuizzes = async () => {
    try {
        ...
        console.log('Skipping seed (Tables might be missing)');
    }
};
...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    // FIX: Using for...of loop to ensure sequential execution (preventing DB locks)
    ...
    // Shopping Tables (Phase 3 Updated)
    ...
    await seedQuizzes(); 
    res.send('<h1>Oneflow Life System Ready (Phase 3 Fixed) 🚀</h1>');
  } catch (err) { 
      console.error(err);
    ...
    try { 
        ...
        if(estimatedPrice !== undefined) {
            ...
            if(itemRes.rows.length > 0 && priceVal > 0) {
                ...
                `, [itemRes.rows[0].item_name, priceVal]);

                if (history.rows.length > 0) {
                    const dateStr = new Date(history.rows[0].date).toLocaleDateString('he-IL');
                    alert = {
                        msg: `נמצא זול יותר: ₪${history.rows[0].price} ב-${history.rows[0].store_name} (${dateStr})`,
                        price: history.rows[0].price
                    };
                }
            }
        }
...
app.post('/api/tasks/update', async (req, res) => {
    ...
    try {
        ...
        // Auto-approve logic for self-tasks or 0 reward
