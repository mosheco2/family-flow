...
// --- Setup DB & Content Injection (Full Batch 2 Support) ---
  ...
  try {
    ...
    if (parseInt(quizCount.rows[0].count) === 0) {
        ...
        const content = [
            ...
            generateMathSet(8, 10, '+', 20), generateMathSet(8, 10, '-', 20),
            generateMathSet(9, 12, '*', 0),
            {
                title: "הבנת הנקרא: הטיול ליער",
                min: 6, max: 9, cat: 'reading', rew: 10,
                questions: [
                    { content: "ביום שבת יצאו דני וגלי לטיול ביער. הם ראו צב קטן הולך לאט.", q: "לאן יצאו הילדים?", opts: ["לים", "ליער", "לקניון", "לבית הספר"], correct: 1 },
                    { content: "המשך...", q: "איזה בעל חיים הם ראו?", opts: ["אריה", "פיל", "צב", "ארנב"], correct: 2 }
                ]
            },
            {
                title: "פיננסים: חיסכון נבון",
    ...
    res.send(`<h2 style="color: green;">System Updated 2.2</h2><p>Content injected and schema migrated.</p>`);
...
app.get('/api/data/:userId', async (req, res) => {
  ...
  try {
    ...
    // Always fetch family for parents or spy mode
    ...
    if (user.role === 'child') {
        const age = new Date().getFullYear() - (user.birth_year || 2015);
        // Assigned Quizzes
        ...
        // Pool Quizzes (Age appropriate & Not done)
        const general = await client.query(`SELECT q.*, NULL as assignment_id, 'pool' as origin FROM quizzes q WHERE $1 >= q.target_min_age AND $1 <= q.target_max_age AND NOT EXISTS (SELECT 1 FROM user_quiz_history h WHERE h.quiz_id = q.id AND h.user_id = $2) ORDER BY RANDOM() LIMIT 10`, [age, userId]);
        quizzes = [...assigned.rows, ...general.rows];
    } else {
        // Parent sees all quizzes to assign
        quizzes = (await client.query('SELECT * FROM quizzes ORDER BY category, target_min_age')).rows;
    }

    let tQ = `SELECT t.*, u.name as user_name FROM transactions t LEFT JOIN users u ON t.user_id = u.id`; if (user.role === 'child') tQ += ` WHERE t.user_id = ${userId}`; tQ += ` ORDER BY t.date DESC LIMIT 50`;
    let taskQ = `SELECT t.*, u.name as assignee_name FROM tasks t LEFT JOIN users u ON t.assigned_to = u.id `; if (user.role === 'child') taskQ += ` WHERE t.assigned_to = ${userId} AND t.status != 'approved'`; else taskQ += ` WHERE t.status != 'approved'`; taskQ += ` ORDER BY t.id DESC`;
