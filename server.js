...
// Financial Concepts (17+)
const FINANCIAL_CONCEPTS = [
    {
        title: "ריבית דריבית",
        text: "ריבית דריבית היא מצב בו הריבית מצטרפת לקרן, ובתקופה הבאה גם היא נושאת ריבית. ככל שהזמן עובר, הכסף גדל בצורה מעריכית.",
        questions: [
            { q: "מהי ריבית דריבית?", options: ["ריבית על הקרן והריבית שנצברה", "ריבית קבועה", "עמלה שנתית", "מס הכנסה"], correct: 0 },
            { q: "מה קורה לכסף בחיסכון לאורך זמן?", options: ["הוא קטן", "גדל לינארית", "גדל מעריכית", "נשאר קבוע"], correct: 2 },
            { q: "מה היתרון העיקרי?", options: ["אפקט הריבית על הריבית", "אין יתרון", "אפשר למשוך מיד", "הריבית נמוכה"], correct: 0 },
            { q: "למי זה מתאים?", options: ["לחיסכון קצר טווח", "לחיסכון ארוך טווח", "להלוואות", "לקניות בסופר"], correct: 1 },
            { q: "מי טבע את המונח 'הפלא השמיני'?", options: ["איינשטיין", "ניוטון", "גלילאו", "בנימין נתניהו"], correct: 0 }
        ]
    }
];
        ...
        for (const age of ages) {
            ...
            if (!isAdult || age === '15-18') {
                for (let i = 1; i <= 3; i++) {
                    ...
                    [`חשבון - רמה ${i}`, age, 3 + Math.floor(Math.random()*5), JSON.stringify(generateMathQuestions(age))]);
                }
            }
            if (isAdult) {
                for (const content of FINANCIAL_CONCEPTS) {
                    await client.query(`INSERT INTO quiz_bundles (title, type, age_group, reward, threshold, text_content, questions) VALUES ($1, 'financial', $2, $3, 80, $4, $5)`, [content.title, age, 10, content.text, JSON.stringify(content.questions)]);
                }
            } else {
                const relevantContent = READING_MATERIALS.find(m => m.age.includes(age));
                if (relevantContent) {
                    await client.query(`INSERT INTO quiz_bundles (title, type, age_group, reward, threshold, text_content, questions) VALUES ($1, 'reading', $2, $3, 80, $4, $5)`, [relevantContent.title, age, 5, relevantContent.text, JSON.stringify(relevantContent.questions)]);
                }
            }
        }
...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    await seedQuizzes();
    res.send('<h1>Oneflow Life System Ready 🚀 (DB Reset Complete)</h1><a href="/">Go Home</a>');
...
app.post('/api/groups', async (req, res) => {
  ...
  try {
    ...
    // Initialize Admin Budgets
    await initBudgets(groupId, null); // Global budget
    await initBudgets(groupId, uRes.rows[0].id); // Personal budget
