...
// --- Setup DB (ללא שינוי, רק לוודא שהכל קיים) ---
app.get('/setup-db', async (req, res) => {
  try {
    ...
    // בדיקת שדרוג עמודות
    try { await client.query("ALTER TABLE goals ADD COLUMN IF NOT EXISTS target_date TIMESTAMP"); } catch(e) {}

    const userCheck = await client.query('SELECT * FROM users');
    if (userCheck.rows.length === 0) await client.query(`INSERT INTO users (name, role, balance, pin_code, age_group) VALUES ('Admin Parent', 'parent', 0, '1234', 'adult')`);
    res.send(`<h2 style="color: green;">System Ready.</h2>`);
  } catch (err) { res.status(500).send(`Error: ${err.message}`); }
});

// --- API Endpoints ---

// Data Endpoint (משודרג עם Leaderboard והיסטוריה מוגדלת)
app.get('/api/data/:userId', async (req, res) => {
  ...
  try {
    ...
    let familyMembers = [];
    // חישוב Leaderboard: מי חסך הכי הרבה החודש? (הכנסות פחות הוצאות)
    let leaderboard = [];
    ...
    // הגדלת היסטוריה ל-50 לצורך חישוב תחזית מדויק יותר
    ...
    res.json({ 
        ...
        goals: goalsRes.rows,
        loans: loansRes.rows,
        leaderboard: leaderboard // שליחת הנתונים החדשים
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// --- שאר ה-API (ללא שינוי, העתק מלא) ---
