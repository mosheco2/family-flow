...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    // 3. הזרקת שאלות (Seeding) - בודקים אם יש שאלות, אם לא - מכניסים
    ...
    if (parseInt(quizCount) === 0) {
        ...
        for (const q of quizzes) {
            await client.query(
                `INSERT INTO quizzes (type, question, content, options, correct_index, reward, target_age_group) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
                q
            );
        }
    }
...
app.post('/api/create-user', async (req, res) => {
    ...
    try {
        // וידוא שהגיל נשמר נכון (ברירת מחדל: ילד קטן אם לא נבחר)
        const age = ageGroup || (role === 'parent' ? 'adult' : 'child_6_10');
        await client.query(`INSERT INTO users (name, role, balance, pin_code, age_group, weekly_allowance, interest_rate) VALUES ($1, $2, $3, $4, $5, 0, 0)`, 
            [name, role, parseFloat(initialBalance)||0, pin, age]);
        await client.query(`INSERT INTO activity_log (user_id, action, icon) VALUES (NULL, $1, 'user-plus')`, [`משתמש חדש: ${name}`]);
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: err.message }); }
});

// שליפת נתונים (מותאם לאקדמיה)
app.get('/api/data/:userId', async (req, res) => {
  ...
  try {
    ...
    const feedRes = await client.query(`SELECT a.*, u.name as user_name FROM activity_log a LEFT JOIN users u ON a.user_id = u.id ORDER BY a.created_at DESC LIMIT 20`);

    // שליפת חידונים - לוגיקה חכמה
    let quizzes = [];
    if (user.role === 'child') {
        // ילד רואה חידונים שמתאימים לגילו (או לכולם) ושטרם עשה
