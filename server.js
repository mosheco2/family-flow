...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    await seedQuizzes();
    res.send(`<h1 style="color:blue">FamilyFlow V6.2 - System Ready & Seeded 🚀</h1>`);
...
app.get('/api/data/:userId', async (req, res) => {
  try {
    ...
    if (user.role === 'ADMIN') { goalsSql = `SELECT g.*, u.nickname as owner_name FROM goals g JOIN users u ON g.user_id = u.id WHERE g.group_id = $1 AND g.status = 'active' ORDER BY g.created_at DESC`; goalsParams = [gid]; } else { goalsSql = `SELECT g.*, u.nickname as owner_name FROM goals g JOIN users u ON g.user_id = u.id WHERE g.user_id = $1 AND g.status = 'active' ORDER BY g.created_at DESC`; goalsParams = [user.id]; }
    ...
    const totalAvailable = parseFloat(user.balance) + weeklyExpenses; // Simplified for UI
    const allowedSpending = totalAvailable * 0.20;
    res.json({ user, tasks: tasks.rows, shopping_list: shop.rows, loans: loans.rows, goals: goalsList, weekly_stats: { spent: weeklyExpenses, limit: allowedSpending }, academyHistory, assignments });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/budget/filter', async (req, res) => {
  ...
  try {
    ...
    const budgetStatus = [];
    if(targetUserId === 'all') { const allocationsTotal = await client.query(`SELECT SUM(amount) as total FROM transactions t JOIN users u ON t.user_id = u.id WHERE u.group_id = $1 AND u.role != 'ADMIN' AND t.type = 'income' AND t.is_manual = FALSE AND (t.category = 'allowance' OR t.category = 'salary' OR t.category = 'bonus') AND date_trunc('month', t.date) = date_trunc('month', CURRENT_DATE)`, [groupId]); budgetStatus.push({ category: 'allocations', label: 'הפרשות לילדים 👶', limit: 0, spent: parseFloat(allocationsTotal.rows[0].total || 0) }); }
    for (const b of budgets.rows) {
      let spentQuery = '', spentParams = [];
      if (targetUserId && targetUserId !== 'all') { spentQuery = `SELECT SUM(amount) as total FROM transactions WHERE user_id = $1 AND category = $2 AND type = 'expense' AND date_trunc('month', date) = date_trunc('month', CURRENT_DATE)`; spentParams = [targetUserId, b.category]; } else { spentQuery = `SELECT SUM(amount) as total FROM transactions t JOIN users u ON t.user_id = u.id WHERE u.group_id = $1 AND t.category = $2 AND t.type = 'expense' AND date_trunc('month', t.date) = date_trunc('month', CURRENT_DATE)`; spentParams = [groupId, b.category]; }
