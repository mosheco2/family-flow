...
app.get('/api/superadmin/data', verifySA, async (req, res) => {
    try {
        ...
        activity.rows.forEach(a => { 
            unifiedActivity.push({ date: a.date, group_name: a.group_name, user_name: a.user_name, description: a.description, amount: a.amount, is_financial: true }); 
        });
...
app.post('/api/admin/send-credentials', async (req, res) => {
    try {
        ...
        usersRes.rows.forEach(u => {
            let roleStr = '';
            if (groupType === 'BUSINESS') { roleStr = u.role === 'ADMIN' ? 'מנהל צוות' : 'עובד'; } else { roleStr = u.role === 'ADMIN' ? 'מנהל/הורה' : 'ילד/בן משפחה'; }
        ...
        if (process.env.SMTP_USER && process.env.SMTP_PASS) {
            ...
            await transporter.sendMail({ from: `"Oneflow" <${process.env.SMTP_USER}>`, to: adminEmail, subject: 'Oneflow - פרטי גישה של משתמשי הסביבה', text: emailContent });
        }
        res.json({ success: true });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/group/:groupId/report-360', async (req, res) => {
    try {
        const { groupId } = req.params; const { adminId } = req.query;
        const adminCheck = await pool.query("SELECT role FROM users WHERE id = $1 AND group_id = $2", [adminId, groupId]);
        if (adminCheck.rows.length === 0 || adminCheck.rows[0].role !== 'ADMIN') return res.status(403).json({error: "רק מנהל הסביבה מורשה להפיק דוח זה"});

        const gRes = await pool.query('SELECT * FROM family_groups WHERE id = $1', [groupId]); const group = gRes.rows[0];
        const uRes = await pool.query('SELECT nickname, role, balance, allowance_amount FROM users WHERE group_id = $1 ORDER BY role, nickname', [groupId]);
        const tRes = await pool.query(`SELECT t.amount, t.type, t.category, t.description, t.date, u.nickname as user_name FROM transactions t LEFT JOIN users u ON t.user_id = u.id WHERE t.group_id = $1 AND t.date >= CURRENT_DATE - INTERVAL '30 days' ORDER BY t.date DESC`, [groupId]);
        const tasksRes = await pool.query('SELECT status, count(*) FROM tasks WHERE group_id = $1 GROUP BY status', [groupId]);
...
app.get('/api/data/:userId', async (req, res) => {
    try {
        ...
        // חישוב מאזן כללי לכל הארגון/משפחה
        ...
        // שליפת רשימת קניות + הצלבה לחוכמת ההמונים
        ...
        for (let item of shoppingList.rows) {
            ...
            if (bestPriceRes.rows.length > 0) {
                ...
                item.best_price = { price_per_unit: bp.price_per_unit, store_name: bp.store_name || 'ספק לא ידוע', branch_name: bp.branch_name || '', trip_date: bp.trip_date, is_local: bp.group_id === group.id };
            }
        }
...
app.post('/api/shopping/checkout', async (req, res) => {
    try {
        ...
        const uRes = await pool.query('SELECT group_id FROM users WHERE id=$1', [userId]); const groupId = uRes.rows[0].group_id;
        ...
        for (let item of boughtItems) {
            ...
            if (pRes.rows.length > 0) { await pool.query(`UPDATE pantry SET quantity = quantity + $1, updated_at = CURRENT_TIMESTAMP WHERE id=$2`, [item.quantity, pRes.rows[0].id]); } 
            else { await pool.query(`INSERT INTO pantry (group_id, item_name, quantity) VALUES ($1, $2, $3)`, [groupId, item.name, item.quantity]); }
...
app.get('/api/shopping/history', async (req, res) => {
    try {
        const { groupId } = req.query; const trips = await pool.query('SELECT st.*, u.nickname FROM shopping_trips st LEFT JOIN users u ON st.buyer_id = u.id WHERE st.group_id=$1 ORDER BY st.trip_date DESC', [groupId]);
...
app.post('/api/shopping/copy', async (req, res) => {
    try {
        ...
        for(let i of items.rows) { await pool.query(`INSERT INTO shopping_list (group_id, requester_id, item_name, quantity, status) VALUES ($1, $2, $3, $4, 'pending')`, [groupId, userId, i.item_name, i.quantity]); }
        res.json({ success: true });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/pantry/add', async (req, res) => {
    try {
        ...
        if (existing.rows.length > 0) { await pool.query('UPDATE pantry SET quantity = quantity + $1, updated_at=CURRENT_TIMESTAMP WHERE id=$2', [parseFloat(quantity) || 1, existing.rows[0].id]); } 
        else { await pool.query('INSERT INTO pantry (group_id, item_name, quantity, unit, units_per_package) VALUES ($1, $2, $3, $4, $5)', [actualGroupId, itemName, parseFloat(quantity) || 1, unit || 'יח\'', parseInt(unitsPerPackage) || 1]); }
        res.json({ success: true });
    } catch(e) { res.status(500).json({ success: false, error: e.message }); }
});
...
app.post('/api/pantry/update', async (req, res) => {
    try {
        const { itemId, quantity } = req.body; await pool.query('UPDATE pantry SET quantity=$1, updated_at=CURRENT_TIMESTAMP WHERE id=$2', [parseFloat(quantity) || 0, itemId]); res.json({ success: true });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/pantry/use', async (req, res) => {
    try {
        ...
        const item = pRes.rows[0]; let deductAmount = 0;
...
app.get('/api/budget/filter', async (req, res) => {
    try {
        const { groupId, targetUserId } = req.query; let params = [groupId]; let targetFilterA = ''; let targetFilterE = '';
        ...
        const query = `
            ...
            SELECT COALESCE(a.category, e.category) as category, COALESCE(MAX(a.amount_limit), 0) as limit, COALESCE(MAX(e.spent), 0) as spent FROM Allocations a FULL OUTER JOIN Expenses e ON a.category = e.category GROUP BY COALESCE(a.category, e.category)
        `;
...
app.post('/api/budget/update', async (req, res) => {
    try {
        const { groupId, category, limit, targetUserId } = req.body; const finalUserId = targetUserId === 'all' ? null : targetUserId;
...
app.get('/api/transactions', async (req, res) => {
    try {
        const { groupId, userId, limit } = req.query; let q = `SELECT t.*, u.nickname as user_name FROM transactions t LEFT JOIN users u ON t.user_id = u.id WHERE t.group_id = $1`; let p = [groupId];
...
app.delete('/api/transaction/:id', async (req, res) => {
    try {
        ...
        if(txRes.rows.length===0) return res.status(404).json({error:'Not found'}); const tx = txRes.rows[0];
...
app.get('/api/loans', async (req, res) => {
    try { const loans = await pool.query('SELECT l.*, u.nickname FROM loans l JOIN users u ON l.user_id = u.id WHERE l.group_id=$1 ORDER BY l.created_at DESC', [req.query.groupId]); res.json(loans.rows); } 
    catch(e) { res.status(500).json({error: e.message}); }
});

app.post('/api/loans/request', async (req, res) => {
    try { const { userId, amount, reason, groupId } = req.body; await pool.query('INSERT INTO loans (user_id, group_id, original_amount, remaining_amount, reason) VALUES ($1, $2, $3, $4, $5)', [userId, groupId, parseFloat(amount)||0, parseFloat(amount)||0, reason]); res.json({success:true}); } 
    catch(e) { res.status(500).json({error: e.message}); }
});

app.post('/api/loans/approve', async (req, res) => {
    try {
        const { loanId, userId, amount, adminId } = req.body; const l = await pool.query('SELECT group_id FROM loans WHERE id=$1', [loanId]);
        await pool.query('UPDATE loans SET status=$1 WHERE id=$2', ['approved', loanId]); await pool.query('UPDATE users SET balance = balance + $1 WHERE id=$2', [parseFloat(amount)||0, userId]);
...
app.post('/api/loans/reject', async (req, res) => {
    try { await pool.query('UPDATE loans SET status=$1 WHERE id=$2', ['rejected', req.body.loanId]); res.json({success:true}); } 
    catch(e) { res.status(500).json({error: e.message}); }
});

app.post('/api/goals', async (req, res) => {
    try { const { userId, targetUserId, title, target, groupId } = req.body; await pool.query('INSERT INTO goals (user_id, target_user_id, title, target_amount) VALUES ($1, $2, $3, $4)', [userId, targetUserId || null, title, parseFloat(target)||0]); res.json({success:true}); } 
    catch(e) { res.status(500).json({error: e.message}); }
});

app.post('/api/goals/deposit', async (req, res) => {
    try {
        const { userId, goalId, amount, groupId } = req.body; await pool.query('BEGIN'); await pool.query('UPDATE users SET balance = balance - $1 WHERE id=$2', [parseFloat(amount)||0, userId]);
...
app.get('/api/group/members', async (req, res) => {
    try { const { groupId } = req.query; const users = await pool.query('SELECT id, nickname, role, balance, allowance_amount, interest_rate, birth_year FROM users WHERE group_id=$1 AND status=$2 ORDER BY role, nickname', [groupId, 'active']); res.json(users.rows); } 
    catch(e) { res.status(500).json({error: e.message}); }
});

app.get('/api/admin/pending-users', async (req, res) => {
    try { const { groupId } = req.query; const users = await pool.query('SELECT id, nickname, role, birth_year FROM users WHERE group_id=$1 AND status=$2', [groupId, 'pending']); res.json(users.rows); } 
    catch(e) { res.status(500).json({error: e.message}); }
});

app.post('/api/admin/approve-user', async (req, res) => {
    try { await pool.query('UPDATE users SET status=$1 WHERE id=$2', ['active', req.body.userId]); res.json({success:true}); } 
    catch(e) { res.status(500).json({error: e.message}); }
});
...
app.post('/api/users/:id/password', async (req, res) => {
    try {
        const { oldPassword, newPassword } = req.body; const u = await pool.query('SELECT password_hash FROM users WHERE id=$1', [req.params.id]);
        if(u.rows[0].password_hash !== oldPassword) return res.status(401).json({error: 'סיסמה נוכחית שגויה'});
        await pool.query('UPDATE users SET password_hash=$1 WHERE id=$2', [newPassword, req.params.id]); res.json({success:true});
    } catch(e) { res.status(500).json({error: e.message}); }
});

app.post('/api/admin/adjust-balance', async (req, res) => {
    try {
        const { adminId, groupId, childId, type, amount, reason } = req.body; const actAmount = type === 'deduct' ? -(parseFloat(amount)||0) : (parseFloat(amount)||0);
...
app.post('/api/admin/payday', async (req, res) => {
    try {
        const { groupId } = req.body; const users = await pool.query(`SELECT id, allowance_amount, interest_rate, balance FROM users WHERE group_id=$1 AND status='active' AND role != 'ADMIN'`, [groupId]);
        let totalDistributed = 0; await pool.query('BEGIN');
...
app.post('/api/tasks', async (req, res) => {
    try {
        const { title, reward, assignedTo, days, status, groupId } = req.body; const deadline = days ? new Date(Date.now() + days * 24 * 60 * 60 * 1000) : null;
...
app.post('/api/tasks/update', async (req, res) => {
    try {
        const { taskId, status, finalReward } = req.body; const tRes = await pool.query('SELECT * FROM tasks WHERE id=$1', [taskId]); const t = tRes.rows[0];
        const rew = finalReward !== undefined ? (parseFloat(finalReward)||0) : (parseFloat(t.reward)||0);
        await pool.query('BEGIN'); await pool.query('UPDATE tasks SET status=$1, reward=$2 WHERE id=$3', [status, rew, taskId]);
        if (status === 'approved') {
...
app.post('/api/academy/assign', async (req, res) => {
    try { const { userId, bundleId, reward, days, groupId } = req.body; const deadline = days ? new Date(Date.now() + days * 24 * 60 * 60 * 1000) : null; await pool.query('INSERT INTO user_assignments (user_id, bundle_id, custom_reward, deadline) VALUES ($1, $2, $3, $4)', [userId, bundleId, reward || null, deadline]); res.json({success:true}); } 
    catch(e) { res.status(500).json({error: e.message}); }
});

app.post('/api/academy/submit', async (req, res) => {
    try {
        const { userId, bundleId, score, groupId } = req.body; const b = await pool.query('SELECT threshold, reward as default_reward FROM quiz_bundles WHERE id=$1', [bundleId]);
...
app.post('/api/academy/ai-generate', async (req, res) => {
    try {
        const { ageGroup, topic, groupId } = req.body; const hasTokens = await handleAITokens(groupId); if(!hasTokens) return res.json({ success: false, error: 'BATTERY_EMPTY' });
        if (!genAI) throw new Error('GEMINI_API_KEY is not set');
        const gRes = await pool.query('SELECT type FROM family_groups WHERE id=$1', [groupId]); const gType = gRes.rows.length > 0 ? gRes.rows[0].type : 'FAMILY';
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash", generationConfig: { responseMimeType: "application/json" } });
        let prompt = "";
        if (gType === 'BUSINESS') { prompt = `Create a professional 5-question multiple-choice training/onboarding quiz in Hebrew about "${topic}" for employees. Requirements: 1. Language MUST be Hebrew. 2. Output strictly as JSON matching this schema: { "title": "A clear title for the training", "text_content": "A short professional text before the questions.", "questions": [ { "q": "The question text", "options": ["Opt 1", "Opt 2", "Opt 3", "Opt 4"], "correct": 0 } ] }`; } 
        else { prompt = `Create a fun and educational 5-question multiple-choice quiz in Hebrew about "${topic}" for children aged ${ageGroup}. Requirements: 1. Language MUST be Hebrew. 2. Output strictly as JSON matching this schema: { "title": "A catchy title for the quiz", "text_content": "A short educational text before the questions. Make it engaging.", "questions": [ { "q": "The question text", "options": ["Opt 1", "Opt 2", "Opt 3", "Opt 4"], "correct": 0 } ] }`; }
        const result = await model.generateContent(prompt); const quizData = JSON.parse(result.response.text()); const bundleType = gType === 'BUSINESS' ? 'professional' : 'financial';
...
app.post('/api/tasks/ai-generate', async (req, res) => {
    try {
        const { age, topic, groupId } = req.body; const hasTokens = await handleAITokens(groupId); if(!hasTokens) return res.json({ success: false, error: 'BATTERY_EMPTY' });
        if (!genAI) throw new Error('GEMINI_API_KEY is not set');
        const gRes = await pool.query('SELECT type FROM family_groups WHERE id=$1', [groupId]); const gType = gRes.rows.length > 0 ? gRes.rows[0].type : 'FAMILY';
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash", generationConfig: { responseMimeType: "application/json" } });
        let prompt = "";
        if (gType === 'BUSINESS') { prompt = `You are an expert organizational manager. Suggest 3 professional tasks, project milestones, or office duties related to the topic: "${topic}". For each task, suggest a fair bonus reward in ILS (integer between 50 and 500). Output STRICTLY as a JSON array of objects without any markdown blocks. Example: [{"title": "task 1", "reward": 100}, {"title": "task 2", "reward": 200}]`; } 
        else { prompt = `You are an expert in parenting. Suggest 3 age-appropriate household chores or educational tasks for a child aged ${age} related to the topic: "${topic}". For each task, suggest a fair monetary reward in ILS (integer between 5 and 50). Output STRICTLY as a JSON array of objects without any markdown blocks. Example: [{"title": "task 1", "reward": 10}, {"title": "task 2", "reward": 20}]`; }
        const result = await model.generateContent(prompt); let textResult = result.response.text(); let parsedTasks;
        try { parsedTasks = JSON.parse(textResult); if (!Array.isArray(parsedTasks)) { if (parsedTasks.tasks && Array.isArray(parsedTasks.tasks)) parsedTasks = parsedTasks.tasks; else parsedTasks = Object.values(parsedTasks).find(val => Array.isArray(val)) || []; } } 
...
app.post('/api/goals/familai-advice', async (req, res) => {
    try {
        const { userId, goalId, groupId } = req.body; const hasTokens = await handleAITokens(groupId); if(!hasTokens) return res.json({ success: false, error: 'BATTERY_EMPTY' });
        if (!genAI) throw new Error('GEMINI_API_KEY is not set');
        const gRes = await pool.query('SELECT type FROM family_groups WHERE id=$1', [groupId]); const gType = gRes.rows.length > 0 ? gRes.rows[0].type : 'FAMILY';
        const userRes = await pool.query('SELECT nickname, birth_year, balance, allowance_amount FROM users WHERE id=$1', [userId]); const goalRes = await pool.query('SELECT title, target_amount, current_amount FROM goals WHERE id=$1', [goalId]);
...
app.post('/api/budget/familai-insight', async (req, res) => {
    try {
        const { groupId } = req.body; const hasTokens = await handleAITokens(groupId); if(!hasTokens) return res.json({ success: false, error: 'BATTERY_EMPTY' });
        if (!genAI) throw new Error('GEMINI_API_KEY is not set');
        const gRes = await pool.query('SELECT type FROM family_groups WHERE id=$1', [groupId]); const gType = gRes.rows.length > 0 ? gRes.rows[0].type : 'FAMILY';
...
app.post('/api/forecast/familai-insight', async (req, res) => {
    try {
        const { groupId, period, mode, targetUserId } = req.body; const hasTokens = await handleAITokens(groupId); if(!hasTokens) return res.json({ success: false, error: 'BATTERY_EMPTY' });
        if (!genAI) throw new Error('GEMINI_API_KEY is not set');
        const gRes = await pool.query('SELECT type FROM family_groups WHERE id=$1', [groupId]); const gType = gRes.rows.length > 0 ? gRes.rows[0].type : 'FAMILY';
        let txsRes;
        ...
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        let prompt = "";
        if (gType === 'BUSINESS') { prompt = `You are a corporate financial advisor. Based on these recurring operational transactions expected for the upcoming ${mode === 'monthly' ? 'month' : 'year'}: ${JSON.stringify(txsRes.rows)}, give a short 2-3 sentence advice in Hebrew on how to prepare and balance the organization's cashflow. Use emojis. Do not use Markdown format.`; } 
        else { prompt = `You are 'familAI', a financial advisor. Based on these recurring transactions expected for the upcoming ${mode === 'monthly' ? 'month' : 'year'}: ${JSON.stringify(txsRes.rows)}, give a short 2-3 sentence advice in Hebrew on how to prepare and balance their cashflow. Use emojis. Do not use Markdown format.`; }
        const result = await model.generateContent(prompt); res.json({ success: true, insight: result.response.text().trim() });
...
app.post('/api/tasks/vision-verify', async (req, res) => {
    try {
        const { taskId, title, imageBase64, mimeType, groupId } = req.body; const hasTokens = await handleAITokens(groupId); if(!hasTokens) return res.json({ success: false, error: 'BATTERY_EMPTY' });
        if (!genAI) throw new Error('GEMINI_API_KEY is not set');
        const gRes = await pool.query('SELECT type FROM family_groups WHERE id=$1', [groupId]); const gType = gRes.rows.length > 0 ? gRes.rows[0].type : 'FAMILY';
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash", generationConfig: { responseMimeType: "application/json" } });
        let prompt = "";
        if (gType === 'BUSINESS') { prompt = `You are an AI QA manager. An employee claims they completed the task/ticket: "${title}". Look at the attached image proof. Is the task reasonably completed? Return JSON strictly matching this schema: { "verified": true/false, "message": "Short feedback in Hebrew speaking directly to the employee. If verified, acknowledge it professionally. If not, clarify what is missing." }`; } 
        ...
        if(feedback.verified) {
            const t = (await pool.query('SELECT * FROM tasks WHERE id=$1', [taskId])).rows[0]; const baseReward = parseFloat(t.reward) || 0; let bonus = 0;
            if(baseReward > 0) { bonus = Math.max(1, Math.round(baseReward * 0.1)); } const total = baseReward + bonus;
...
app.post('/api/shopping/scan-receipt', async (req, res) => {
    try {
        ...
        const result = await model.generateContent([ prompt, { inlineData: { data: imageBase64, mimeType: mimeType || "image/jpeg" } } ]);
        const items = JSON.parse(result.response.text()); let normalizedArray = [];
        try {
            const names = items.map(i => i.name); const normPrompt = `Normalize these product names to generic, brand-less Hebrew names. Return a JSON array of strings in the exact same order. Example: ["חלב תנובה 3%", "במבה אסם 80ג"] -> ["חלב 3%", "במבה"]. Items: ${JSON.stringify(names)}`;
            const normResult = await model.generateContent(normPrompt); normalizedArray = JSON.parse(normResult.response.text());
...
app.post('/api/academy/tutor', async (req, res) => {
    try {
        const { question, wrongAnswer, correctAnswer, groupId } = req.body; const hasTokens = await handleAITokens(groupId); if(!hasTokens) return res.json({ success: false, error: 'BATTERY_EMPTY' });
        if (!genAI) throw new Error('GEMINI_API_KEY is not set');
        const gRes = await pool.query('SELECT type FROM family_groups WHERE id=$1', [groupId]); const gType = gRes.rows.length > 0 ? gRes.rows[0].type : 'FAMILY';
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        let prompt = "";
        if (gType === 'BUSINESS') { prompt = `You are a professional corporate trainer. An employee answered a training question incorrectly. Question: "${question}" They answered: "${wrongAnswer}" The correct answer is: "${correctAnswer}". Explain briefly and professionally in Hebrew (2-3 sentences max) why the correct answer is right. Be constructive.`; } 
...
app.post('/api/guide/chat', async (req, res) => {
    try {
        const { question } = req.body; if (!genAI) return res.status(500).json({ success: false, error: 'מפתח API חסר בשרת' });
        let guideText = "";
        try { guideText = fs.readFileSync(path.join(__dirname, 'public', 'guide.html'), 'utf-8'); } catch(e) { guideText = "Oneflow Life is a family financial and task management app."; }
        const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        const prompt = `You are 'familAI', the friendly AI assistant for the 'Oneflow Life' app. A user is reading the user guide and asked a question to understand the system better. Here is the full content of the guide HTML: ${guideText} User's question: "${question}" Answer directly in Hebrew based ONLY on the guide content above. Be concise (3-4 sentences max), friendly, use emojis, and address the user directly. Do not use complex markdown, just basic bolding where needed.`;
        const result = await model.generateContent(prompt); res.json({ success: true, answer: result.response.text().trim() });
    } catch (e) { res.status(500).json({ success: false, error: 'מצטערת, לא הצלחתי לייצר תשובה כרגע.' }); }
});
...
app.get('/api/timeclock/status', async (req, res) => {
    try {
        const { userId } = req.query; const openPunch = await pool.query('SELECT punch_in FROM time_clock WHERE user_id=$1 AND punch_out IS NULL', [userId]);
...
app.post('/api/timeclock/punch', async (req, res) => {
    try {
        ...
        if (!lat || !lng) { return res.status(400).json({ error: 'לא התקבל מיקום. חובה לאשר גישה למיקום (GPS) כדי לדווח.' }); }
        ...
        const bizLat = gRes.rows[0].location_lat; const bizLng = gRes.rows[0].location_lng;
        if (!bizLat || !bizLng) { return res.status(400).json({ error: 'המנהל טרם הגדיר את מיקום העסק במערכת. פנה להנהלה.' }); }
        const distance = calculateDistance(lat, lng, bizLat, bizLng);
        const MAX_ALLOWED_DISTANCE = 150; 
        if (distance > MAX_ALLOWED_DISTANCE) { return res.status(403).json({ error: `אינך נמצא בקרבת העסק. מרחק נוכחי: ${Math.round(distance)} מטר. מותר עד ${MAX_ALLOWED_DISTANCE} מטר.` }); }
        ...
        if (openPunch.rows.length > 0) {
            const punchId = openPunch.rows[0].id; const punchIn = new Date(openPunch.rows[0].punch_in); const punchOut = new Date();
...
app.get('/api/timeclock/report', async (req, res) => {
    try {
        const { groupId, userId } = req.query; let query, params;
        if (userId === 'all') { query = `SELECT tc.*, u.nickname FROM time_clock tc JOIN users u ON tc.user_id = u.id WHERE tc.group_id=$1 ORDER BY tc.punch_in DESC`; params = [groupId]; } 
        else { query = `SELECT tc.*, u.nickname FROM time_clock tc JOIN users u ON tc.user_id = u.id WHERE tc.user_id=$1 ORDER BY tc.punch_in DESC`; params = [userId]; }
        const result = await pool.query(query, params); res.json(result.rows);
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// --- NEW: MANUAL PUNCH ENDPOINT ---
app.post('/api/timeclock/manual', async (req, res) => {
    try {
        const { groupId, userId, punchIn, punchOut, totalMins } = req.body;
        if (!userId || !punchIn || !punchOut) return res.status(400).json({error: 'חסרים נתונים'});
        
        await pool.query(
            'INSERT INTO time_clock (user_id, group_id, punch_in, punch_out, total_minutes) VALUES ($1, $2, $3, $4, $5)',
            [userId, groupId, new Date(punchIn), new Date(punchOut), totalMins]
        );
        res.json({ success: true });
    } catch(e) { 
...
app.post('/api/groups', async (req, res) => {
    let dbClient;
    try {
        dbClient = await pool.connect(); await dbClient.query('BEGIN'); let code = generateGroupCode();
        ...
        const group = gRes.rows[0]; const birthYear = parseInt(req.body.birthYear) || null;
        const uRes = await dbClient.query(`INSERT INTO users (group_id, nickname, birth_year, password_hash, role, status) VALUES ($1, $2, $3, $4, 'ADMIN', 'active') RETURNING *`, [group.id, req.body.adminNickname, birthYear, req.body.password]);
        const welcomeText = req.body.type === 'BUSINESS' ? 'סביבת עבודה נפתחה בהצלחה! 🎉' : 'הבנק המשפחתי נפתח בהצלחה! 🎉';
        await dbClient.query(`INSERT INTO transactions (user_id, group_id, amount, description, category, type, is_manual) VALUES ($1, $2, 0, $3, 'system', 'income', FALSE)`, [uRes.rows[0].id, group.id, welcomeText]);
        await dbClient.query('COMMIT'); res.json({ success: true, user: uRes.rows[0], group: group });
    } catch (e) { 
        if (dbClient) { try { await dbClient.query('ROLLBACK'); } catch(rbErr) { console.error('Rollback failed', rbErr); } }
...
app.post('/api/join', async (req, res) => {
    try {
        ...
        const group = gRes.rows[0]; const reqRole = role === 'ADMIN' ? 'ADMIN' : 'MEMBER'; const bYear = parseInt(birthYear) || null;
        await pool.query(`INSERT INTO users (group_id, nickname, birth_year, password_hash, role, status) VALUES ($1, $2, $3, $4, $5, 'pending')`, [group.id, nickname, bYear, password, reqRole]);
        res.json({ success: true });
...
app.post('/api/login', async (req, res) => {
    try {
        if (!req.body.groupCode || !req.body.nickname || !req.body.password) { return res.status(400).json({ error: 'חסרים פרטי התחברות' }); }
        ...
        const group = gRes.rows[0]; const uRes = await pool.query('SELECT * FROM users WHERE group_id = $1 AND nickname = $2 AND password_hash = $3', [group.id, req.body.nickname, req.body.password]);
