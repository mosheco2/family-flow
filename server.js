...
const seedQuizzes = async () => {
    ...
    console.log('Seeding Content V7.0...');
    const ageGroups = ['6-8', '8-10', '10-13', '13-15', '15-18', '18+'];
    const texts = {
        reading: [
            {
                title: 'ההיסטוריה של הכסף',
                text: `לפני אלפי שנים, לא היה כסף כמו שאנחנו מכירים היום. אנשים השתמשו בשיטה שנקראת "סחר חליפין". אם רצית תפוח, היית צריך לתת משהו בתמורה, כמו ביצה או חלב.`,
                qs: [
...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    await seedQuizzes();
    res.send(`<h1 style="color:green">System Ready V7.0 (Fix) 🛡️</h1>`);
  } catch (err) { res.status(500).send(`Error: ${err.message}`); }
});

// FIXED: Simplified budget init logic to avoid SQL null issues
...
app.post('/api/join', async (req, res) => {
  ...
  try {
    ...
    await initBudgets(gRes.rows[0].id, uRes.rows[0].id); // Create budget rows for member
