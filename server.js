...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    await seedQuizzes(); 
    res.send('<h1>Oneflow Life DB Ready</h1>');
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (user.status !== 'ACTIVE') return res.status(403).json({ error: 'חשבונך ממתין לאישור מנהל' });
...
// --- FIX: Approve User + Init Budget ---
app.post('/api/admin/approve-user', async (req, res) => { 
    try { 
        await client.query("UPDATE users SET status = 'ACTIVE' WHERE id = $1", [req.body.userId]); 
        
        // Critical Fix: Initialize budget for the new user immediately
...
app.post('/api/shopping/copy', async (req, res) => {
    ...
    try {
        ...
        for (const i of items.rows) await client.query(`INSERT INTO shopping_list (item_name, quantity, estimated_price, requested_by, group_id, status) VALUES ($1, $2, $3, $4, $5, 'pending')`, [i.item_name, i.quantity, i.price_per_unit, userId, u.rows[0].group_id]);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// --- OTHER (Tasks, Academy, Budget - Unchanged from base) ---
