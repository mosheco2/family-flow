...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS activity_log (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), action VARCHAR(255) NOT NULL, icon VARCHAR(50) DEFAULT 'bell', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`);
    
    // טבלאות אקדמיה משודרגות
    await client.query(`
        CREATE TABLE IF NOT EXISTS quizzes (
            id SERIAL PRIMARY KEY,
            type VARCHAR(20) NOT NULL, -- trivia, math, reading
            question VARCHAR(500) NOT NULL,
            content TEXT, -- טקסט קריאה (אם רלוונטי)
            options JSONB NOT NULL, -- אפשרויות בחירה
            correct_index INTEGER NOT NULL,
            reward DECIMAL(10, 2) DEFAULT 1,
            target_age_group VARCHAR(20) DEFAULT 'all' -- child_6_10, child_10_15, teen_15_18
        );
    `);
    
    await client.query(`CREATE TABLE IF NOT EXISTS user_quiz_history (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id), quiz_id INTEGER REFERENCES quizzes(id), completed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`);

    // הוספת עמודות חסרות (שדרוג דינמי)
    try {
        await client.query("ALTER TABLE quizzes ADD COLUMN IF NOT EXISTS type VARCHAR(20) DEFAULT 'trivia'");
        await client.query("ALTER TABLE quizzes ADD COLUMN IF NOT EXISTS content TEXT");
        await client.query("ALTER TABLE quizzes ADD COLUMN IF NOT EXISTS target_age_group VARCHAR(20) DEFAULT 'all'");
    } catch(e) {}
    ...
    if (quizCount == 0) {
        // חשבון לקטנטנים (6-10)
        await client.query(`INSERT INTO quizzes (type, question, options, correct_index, reward, target_age_group) VALUES 
            ('math', 'כמה זה 5 + 3?', '["7", "8", "9", "6"]', 1, 2.00, 'child_6_10'),
            ('math', 'כמה זה 10 - 4?', '["5", "6", "4", "7"]', 1, 2.00, 'child_6_10'),
            ('math', 'לדני יש 2 תפוחים, הוא קיבל עוד 3. כמה יש לו?', '["4", "5", "6", "3"]', 1, 3.00, 'child_6_10');
        `);
        
        // קריאה לקטנטנים (6-10)
        await client.query(`INSERT INTO quizzes (type, question, content, options, correct_index, reward, target_age_group) VALUES 
            ('reading', 'מה גלי אהבה לאכול?', 'גלי הלכה למטבח. על השולחן הייתה צלחת גדולה עם פירות. גלי לקחה בננה צהובה ומתוקה ואכלה אותה בהנאה.', '["תפוח", "אגס", "בננה", "ענבים"]', 2, 5.00, 'child_6_10'),
            ('reading', 'איזה צבע היה לכדור?', 'יוסי שיחק בחצר עם הכלב שלו. פתאום, יוסי זרק כדור אדום והכלב רץ להביא אותו.', '["כחול", "אדום", "ירוק", "צהוב"]', 1, 5.00, 'child_6_10');
        `);

        // פיננסים וחשבון לבינוניים (10-15)
        await client.query(`INSERT INTO quizzes (type, question, options, correct_index, reward, target_age_group) VALUES 
            ('trivia', 'מהי ריבית?', '["קנס על איחור", "תשלום על השימוש בכסף", "סוג של מס", "מתנה מהבנק"]', 1, 10.00, 'child_10_15'),
            ('math', 'אם חוסכים 10 שקלים כל שבוע, כמה יהיה אחרי חודש (4 שבועות)?', '["30", "40", "50", "20"]', 1, 5.00, 'child_10_15'),
            ('trivia', 'מה עדיף לעשות עם דמי כיס?', '["לבזבז הכל מיד", "לשמור מתחת לכרית", "לחלק: קצת לבזבוזים וקצת לחיסכון", "לתת לחברים"]', 2, 10.00, 'child_10_15');
        `);
        
        // פיננסים לבוגרים (15+)
        await client.query(`INSERT INTO quizzes (type, question, options, correct_index, reward, target_age_group) VALUES 
            ('trivia', 'מה ההבדל בין כרטיס דביט (Debit) לכרטיס אשראי (Credit)?', '["אין הבדל", "בדביט הכסף יורד מיד, באשראי יורד פעם בחודש", "באשראי אי אפשר להיכנס למינוס", "דביט הוא רק לחו״ל"]', 1, 15.00, 'teen_15_18'),
            ('trivia', 'מה זה "תקציב"?', '["רשימת קניות", "תוכנית לניהול הכנסות והוצאות", "הלוואה מהבנק", "סכום הכסף שיש לי בארנק"]', 1, 15.00, 'teen_15_18');
        `);
    }
    ...
    res.send(`<h2 style="color: green;">System Updated! Content Loaded.</h2>`);
  } catch (err) { res.status(500).send(`Error: ${err.message}`); }
});

// --- פונקציית עזר ללוגים ---
async function logActivity(userId, action, icon = 'bell') {
    try { await client.query(`INSERT INTO activity_log (user_id, action, icon) VALUES ($1, $2, $3)`, [userId, action, icon]); } catch (e) {}
}

// --- API Endpoints ---

// יצירת חידון חדש (להורים)
app.post('/api/academy/create', async (req, res) => {
    const { type, question, content, options, correctIndex, reward, targetAge } = req.body;
    try {
        await client.query(`
            INSERT INTO quizzes (type, question, content, options, correct_index, reward, target_age_group)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
        `, [type, question, content, JSON.stringify(options), correctIndex, reward, targetAge]);
...
app.post('/api/academy/answer', async (req, res) => {
    ...
    try {
        ...
        const quiz = (await client.query('SELECT * FROM quizzes WHERE id = $1', [quizId])).rows[0];
        
        if (parseInt(answerIndex) === quiz.correct_index) {
            ...
            await client.query(`INSERT INTO transactions (user_id, amount, description, category, type) VALUES ($1, $2, $3, 'education', 'income')`, [userId, reward, `הצלחה באקדמיה: ${quiz.question.substring(0, 10)}...`]);
            await client.query('INSERT INTO user_quiz_history (user_id, quiz_id) VALUES ($1, $2)', [userId, quizId]);
            await logActivity(userId, `למד והרוויח ₪${reward}`, 'graduation-cap');
            await client.query('COMMIT');
            res.json({ success: true, correct: true, reward: reward });
        } else {
...
app.get('/api/data/:userId', async (req, res) => {
  ...
  try {
    ...
    // פיד (מביא את הכל - ההורה רוצה לראות הכל, וגם הילדים)
    const feedRes = await client.query(`SELECT a.*, u.name as user_name FROM activity_log a LEFT JOIN users u ON a.user_id = u.id ORDER BY a.created_at DESC LIMIT 30`);
    ...
    if (user.role === 'child') loansQuery += ` WHERE l.user_id = ${userId}`; else loansQuery += ` WHERE l.status != 'paid'`; 
    loansQuery += ` ORDER BY l.created_at DESC`;
    const loansRes = await client.query(loansQuery);

    const lbQuery = await client.query(`SELECT u.name, u.role, COALESCE(SUM(CASE WHEN t.type = 'income' THEN t.amount ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN t.type = 'expense' THEN t.amount ELSE 0 END), 0) as monthly_savings FROM users u LEFT JOIN transactions t ON u.id = t.user_id AND date_trunc('month', t.date) = date_trunc('month', CURRENT_DATE) WHERE u.role = 'child' GROUP BY u.id ORDER BY monthly_savings DESC`);

    res.json({ user, transactions: transRes.rows, family: familyMembers, tasks: tasksRes.rows, shopping_list: shopRes.rows, goals: goalsRes.rows, loans: loansRes.rows, leaderboard: lbQuery.rows, feed: feedRes.rows, quizzes: quizzes });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// --- העתק פונקציות קיימות ---
...
app.post('/api/shopping/checkout', async (req, res) => { const { totalAmount, userId } = req.body; try { await client.query('BEGIN'); await client.query("UPDATE shopping_list SET status = 'bought' WHERE status = 'in_cart'"); await client.query(`INSERT INTO transactions (user_id, amount, description, category, type) VALUES ($1, $2, 'קניות בסופר', 'groceries', 'expense')`, [userId, parseFloat(totalAmount)]); await logActivity(userId, `ביצע קנייה בסופר ב-₪${totalAmount}`, 'cart-shopping'); await client.query('COMMIT'); res.json({ success: true }); } catch (err) { await client.query('ROLLBACK'); res.status(500).json({ error: err.message }); } });
app.post('/api/tasks', async (req, res) => { const { title, reward, assignedTo } = req.body; try { await client.query(`INSERT INTO tasks (title, reward, status, assigned_to) VALUES ($1, $2, 'pending', $3)`, [title, reward, assignedTo]); await logActivity(null, `משימה חדשה: ${title}`, 'list-check'); res.json({ success: true }); } catch (err) { res.status(500).json({ error: err.message }); } });
app.post('/api/tasks/update', async (req, res) => { const { taskId, status } = req.body; try { await client.query('BEGIN'); if (status === 'approved') { const task = (await client.query('SELECT * FROM tasks WHERE id = $1', [taskId])).rows[0]; if (task && task.status !== 'approved') { await client.query(`UPDATE users SET balance = balance + $1 WHERE id = $2`, [task.reward, task.assigned_to]); await client.query(`INSERT INTO transactions (user_id, amount, description, category, type) VALUES ($1, $2, $3, 'tasks', 'income')`, [task.assigned_to, task.reward, `בוצע: ${task.title}`]); await logActivity(task.assigned_to, `סיים משימה: ${task.title}`, 'check'); } } await client.query('UPDATE tasks SET status = $1 WHERE id = $2', [status, taskId]); await client.query('COMMIT'); res.json({ success: true }); } catch (err) { await client.query('ROLLBACK'); res.status(500).json({ error: err.message }); } });
