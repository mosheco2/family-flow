...
// 2. נתיב "בדיקת דופק" (Health Check) - מונע שגיאת 502
app.get('/', (req, res) => {
    const dbStatus = client._connected ? "Connected ✅" : "Disconnected ❌";
    res.send(`
        <h1>Oneflow Life Server is UP! 🚀</h1>
        <p>Status: <strong>Running</strong></p>
        <p>Database: <strong>${dbStatus}</strong></p>
        <hr>
        <p>אם ה-Database מנותק, אנא בדוק את הגדרות ה-Environment Variables ב-Render.</p>
        <p>כדי לאפס את המערכת (כשה-DB מחובר), גש ל: <a href="/setup-db">/setup-db</a></p>
    `);
});

// 3. הגדרת חיבור למסד הנתונים (מוגן מקריסה)
let client;
const dbConfig = {
...
if (!process.env.DATABASE_URL) {
    ...
    // אנחנו יוצרים קליינט פיקטיבי כדי שהשרת יעלה בכל זאת ויציג שגיאה בדפדפן
    client = { query: async () => { throw new Error("DB Not Connected"); }, _connected: false, connect: async ()=>{} };
} else {
...
const seedQuizzes = async () => {
    ...
    } catch (e) { console.log('Seed skipped (DB ready or empty)'); }
};

// --- SETUP DB ROUTE ---
app.get('/setup-db', async (req, res) => {
  if (!client._connected) return res.status(500).send("❌ Database not connected. Check DATABASE_URL.");
  
  try {
    // Drop in order to prevent locks
    ...
    await seedQuizzes(); 
    res.send('<h1>Oneflow Life System Ready 🚀</h1><p>Database initialized successfully.</p><a href="/">Go to App</a>');
...
app.post('/api/shopping/checkout', async (req, res) => {
    ...
    try {
        ...
        for (const i of missingItems) await client.query("UPDATE shopping_list SET status = 'pending' WHERE id = $1", [i.id]);
...
app.get('/api/shopping/history', async (req, res) => {
    try {
        ...
        for (const trip of trips.rows) {
            ...
            data.push({ ...trip, items: items.rows });
        }
        res.json(data);
...
app.post('/api/shopping/copy', async (req, res) => {
    try {
        ...
        for (const item of items.rows) {
            await client.query(`INSERT INTO shopping_list (item_name, quantity, estimated_price, requested_by, group_id, status) VALUES ($1, $2, $3, $4, $5, 'pending')`, [item.item_name, item.quantity, item.price_per_unit || 0, req.body.userId, u.rows[0].group_id]);
        }
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
