...
// --- שדרוג מסד הנתונים (מוודא שכל הטבלאות והעמודות קיימות) ---
app.get('/setup-db', async (req, res) => {
  try {
    ...
    // 2. הוספת עמודות חסרות (למקרה של שדרוג גרסה)
    const columnsToAdd = [
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS age_group VARCHAR(20) DEFAULT 'adult'",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS weekly_allowance DECIMAL(10, 2) DEFAULT 0",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS interest_rate DECIMAL(5, 2) DEFAULT 0",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS xp INTEGER DEFAULT 0",
        "ALTER TABLE goals ADD COLUMN IF NOT EXISTS target_date TIMESTAMP",
    ...
    for (const query of columnsToAdd) {
        try { await client.query(query); } catch(e) { console.log('Column skip/error:', e.message); }
    }

    // 3. יצירת הורה ברירת מחדל
    ...
    res.send(`<h2 style="color: green;">System Fully Upgraded! All modules active. 🚀</h2>`);
...
app.get('/api/data/:userId', async (req, res) => {
  ...
  try {
    ...
    // רק הורים וילדים רואים את המשפחה והלידרבורד
    if (user.role === 'parent' || user.role === 'child') {
        ...
        // חישוב לידרבורד חודשי (חיסכון נטו)
    ...
    transQuery += ` ORDER BY t.date DESC LIMIT 50`; // הגדלנו ל-50 להיסטוריה
...
// ברירת מחדל: שליחת קובץ ה-HTML הראשי (קריטי!)
app.get('*', (req, res) => {
