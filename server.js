...
app.post('/api/academy/request-challenge', async (req, res) => { 
    try { 
        ...
        if (!bundleId) {
            ...
            const age = calculateAge(user.birth_year); const ageGroup = getAgeGroup(age); 
            ...
            if (available.rows.length === 0) return res.json({ success: false, error: 'אין אתגרים' }); 
...
app.get('/api/admin/academy-requests', async (req, res) => {
    try {
        const r = await client.query(`SELECT ua.id, ua.user_id, ua.bundle_id, u.nickname, qb.title, qb.reward, qb.age_group FROM user_assignments ua JOIN users u ON ua.user_id = u.id JOIN quiz_bundles qb ON ua.bundle_id = qb.id WHERE u.group_id = $1 AND ua.status = 'pending_approval'`, [req.query.groupId]);
        res.json(r.rows);
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/academy/approve-request', async (req, res) => {
    try {
        ...
        await client.query(`UPDATE user_assignments SET status='assigned', custom_reward=$1, deadline=$2 WHERE id=$3`, [reward, deadline, assignmentId]);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/academy/assign', async (req, res) => {
    try {
        ...
        const deadline = days ? new Date(Date.now() + days * 86400000) : null;
        await client.query(`INSERT INTO user_assignments (user_id, bundle_id, status, custom_reward, deadline) VALUES ($1, $2, 'assigned', $3, $4)`, [userId, bundleId, reward || null, deadline]);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/academy/submit', async (req, res) => { 
    try { 
        ...
        if (ua.deadline && new Date() > new Date(ua.deadline)) { await client.query(`UPDATE user_assignments SET status='expired', date_completed=NOW() WHERE id=$1`, [ua.id]); await client.query('COMMIT'); return res.json({ success: true, passed: false, expired: true }); }
        ...
        if(passed) { await client.query(`UPDATE users SET balance = balance + $1 WHERE id = $2`, [reward, req.body.userId]); await client.query(`INSERT INTO transactions (user_id, amount, description, category, type, is_manual) VALUES ($1, $2, $3, 'salary', 'income', FALSE)`, [req.body.userId, reward, `בונוס אקדמיה: ${ua.title}`]); } 
        if(!passed) { await client.query(`DELETE FROM user_assignments WHERE id=$1`, [ua.id]); } 
        await client.query('COMMIT'); res.json({ success: true, passed, reward }); 
...
app.post('/api/academy/create-bundle', async (req, res) => {
    try {
        ...
        await client.query(`INSERT INTO quiz_bundles (title, type, age_group, reward, threshold, text_content, questions, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [title, type, ageGroup, reward, threshold, textContent, JSON.stringify(questions), creatorName || 'ADMIN']);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// ... (Tasks, Budget, Loans, etc.)
