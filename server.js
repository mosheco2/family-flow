...
// --- CONTENT GENERATOR (ENHANCED V6.2) ---
const generateMathQuestions = (ageGroup) => {
    const questions = [];
    for (let i = 0; i < 5; i++) {
        let q, a;
        // Math logic adapted for clearer display
        if (ageGroup === '6-8') {
...
const seedQuizzes = async () => {
    ...
    console.log('Seeding Rich Content V6.2...');
...
// FIXED: Init Budgets logic to prevent empty budget tab
  ...
  try {
    ...
    if(user.role !== 'ADMIN') {
        const assignsRes = await client.query(`
            SELECT ua.*, qb.title, qb.type, qb.reward, qb.threshold, ua.deadline 
            FROM user_assignments ua 
            JOIN quiz_bundles qb ON ua.bundle_id = qb.id 
...
// FIXED: Self-healing Budget (Ensure rows exist)
