...
const seedQuizzes = async () => {
    ...
    for (const age of ageGroups) {
        // 1. Generate 50 Math Bundles per age
        for (let i = 1; i <= 50; i++) {
            await client.query(
        ...
        // 2. Generate 50 Reading Bundles (Templates)
        for (let i = 1; i <= 50; i++) {
        ...
        // 3. Generate 50 Financial Bundles
        for (let i = 1; i <= 50; i++) {
...
app.get('/setup-db', async (req, res) => {
  try {
    ...
    // UPDATED: Added deadline and custom_reward to assignment
    ...
    res.send(`<h1 style="color:blue">FamilyFlow V6.1 - System Ready with 300+ Quizzes 🚀</h1>`);
...
// ... [Auth, Banking, Goals logic remains same] ...
// Copying previous stable Auth/Bank logic for brevity, focusing on NEW ACADEMY updates below
// (In production this would be modular, here assuming concatenation of previous stable blocks)

// --- RE-INSERTING STABLE AUTH & BANKING BLOCKS FOR COMPLETENESS ---
...
app.get('/api/data/:userId', async (req, res) => { try { const user = (await client.query('SELECT * FROM users WHERE id = $1', [req.params.userId])).rows[0]; if (!user) return res.status(404).json({ error: 'User not found' }); const gid = user.group_id; let tasksSql = `SELECT t.*, u.nickname as assignee_name FROM tasks t LEFT JOIN users u ON t.assigned_to = u.id WHERE t.group_id = $1`; if(user.role !== 'ADMIN') tasksSql += ` AND t.assigned_to = ${user.id}`; tasksSql += ` ORDER BY t.created_at DESC`; let loansSql = `SELECT l.*, u.nickname as user_name FROM loans l LEFT JOIN users u ON l.user_id = u.id WHERE l.group_id = $1`; if(user.role !== 'ADMIN') loansSql += ` AND l.user_id = ${user.id}`; loansSql += ` ORDER BY l.created_at DESC`; const expensesRes = await client.query(`SELECT SUM(amount) as total FROM transactions WHERE user_id = $1 AND type = 'expense' AND date > NOW() - INTERVAL '7 days'`, [user.id]); const weeklyExpenses = parseFloat(expensesRes.rows[0].total || 0); let goalsSql = ''; let goalsParams = []; if (user.role === 'ADMIN') { goalsSql = `SELECT g.*, u.nickname as owner_name FROM goals g JOIN users u ON g.user_id = u.id WHERE g.group_id = $1 AND g.status = 'active' ORDER BY g.created_at DESC`; goalsParams = [gid]; } else { goalsSql = `SELECT g.*, u.nickname as owner_name FROM goals g JOIN users u ON g.user_id = u.id WHERE g.user_id = $1 AND g.status = 'active' ORDER BY g.created_at DESC`; goalsParams = [user.id]; } const goalsRes = await client.query(goalsSql, goalsParams); const goalsList = goalsRes.rows; const userGoals = goalsList.filter(g => g.user_id === user.id); const goalsTotal = userGoals.reduce((sum, g) => sum + parseFloat(g.current_amount), 0); const totalAvailable = parseFloat(user.balance) + goalsTotal + weeklyExpenses; const allowedSpending = totalAvailable * 0.20; 
    ...
    // --- ADMIN ACADEMY HISTORY ---
    let academyHistory = [];
    if (user.role === 'ADMIN') {
        const histRes = await client.query(`
            SELECT ua.*, u.nickname, qb.title 
...
// ... [Budget filter/update, Transaction, Task, Shopping, Loan APIs remain the same] ...
...
app.get('/api/academy/bundles', async (req, res) => {
    ...
    try { const r = await client.query(sql + ' ORDER BY id LIMIT 100', params); res.json(r.rows); } catch (e) { res.status(500).json({error:e.message}); }
});

app.post('/api/academy/assign', async (req, res) => {
    // UPDATED: Now supports custom reward and deadline
    ...
    try {
        ...
        await client.query(
            `INSERT INTO user_assignments (user_id, bundle_id, status, score, custom_reward, deadline) VALUES ($1, $2, 'assigned', 0, $3, $4)`, 
            [userId, bundleId, customReward || null, deadline]
        );
        res.json({ success: true });
...
app.post('/api/academy/submit', async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        // Check assignment for custom reward and deadline validity
        ...
        if (assignment.deadline && new Date() > new Date(assignment.deadline)) {
            finalStatus = 'late'; // Failed due to time
            finalReward = 0; // No reward if late (can be changed logic)
        }
        ...
        if (finalStatus === 'completed') {
            ...
            await client.query(`INSERT INTO transactions (user_id, amount, description, category, type, is_manual) VALUES ($1, $2, $3, 'salary', 'income', FALSE)`, [userId, finalReward, `בונוס אקדמיה: ${title}`]);
        }
        await client.query('COMMIT');
...
app.post('/api/academy/request-challenge', async (req, res) => { const { userId } = req.body; res.json({ success: true }); });
