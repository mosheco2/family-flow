...
test.describe('רשימת קניות (FAM-03..06)', () => {
  ...
  test('[FAM-04] עריכת פריט ברשימת קניות', async ({ page }) => {
    ...
    if (!hasBtn) {
      console.warn('[FAM-04] כפתור עריכה לא נמצא — ייתכן שאין פריטים ברשימה; FAM-03 חייב לרוץ קודם');
      await expect(page.locator('#content-shop')).toBeVisible({ timeout: 6000 });
      return;
    }
    await editBtn.click();
...
test.describe('מזווה (FAM-08..09)', () => {
  ...
  test('[FAM-09] שימוש/הפחתת כמות מפריט במזווה', async ({ page }) => {
    ...
    await goToTab(page, 'pantry');
    await page.waitForTimeout(1500);
    // כפתור "השתמש" — soft assertion (תלוי בקיום פריטים מ-FAM-08)
    ...
    const hasBtn = await useBtn.isVisible({ timeout: 5000 }).catch(() => false);
    if (!hasBtn) {
      console.warn('[FAM-09] כפתור השתמש לא נמצא — ייתכן שאין פריטים במזווה; FAM-08 חייב לרוץ קודם');
