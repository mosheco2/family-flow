...
test.describe('ניהול תכנים (ACAD-05..07)', () => {
  ...
  test('[ACAD-05] אקדמיה — הוספת הכשרה חדשה', async ({ page }) => {
    ...
    await goToTab(page, 'academy');
    await page.waitForTimeout(1200);
    const addBtn = page.locator('button:has-text("✨ יצירת אתגר"), button:has-text("יצירת אתגר"), button:has-text("הקצאה"), button:has-text("+ הכשרה"), button:has-text("הכשרה חדשה"), button:has-text("הוסף תוכן")').first();
    const hasBtn = await addBtn.isVisible({ timeout: 6000 }).catch(() => false);
    if (hasBtn) {
      await addBtn.click();
      await page.waitForTimeout(600);
      const modal = page.locator('[id*="academy"][id*="modal"], [id*="course"][id*="modal"], [id*="training"][id*="modal"], #ai-modal, #assign-quiz-modal').first();
      const isOpen = await modal.isVisible({ timeout: 5000 }).catch(() => false);
      if (!isOpen) console.warn('[ACAD-05] מודל הוספת הכשרה לא נפתח');
    } else {
      console.warn('[ACAD-05] כפתור הוספת הכשרה לא נמצא');
    }
  ...
  test('[ACAD-06] אקדמיה — הקצאת הכשרה לעובד', async ({ page }) => {
    ...
    const assignBtn = page.locator('button:has-text("הקצאה"), button:has-text("הקצה"), button[onclick*="openAssignModal"], button:has-text("שלח הכשרה"), .assign-btn').first();
    const hasBtn = await assignBtn.isVisible({ timeout: 6000 }).catch(() => false);
    if (!hasBtn) console.warn('[ACAD-06] כפתור הקצאת הכשרה לא נמצא');
    await expect(page.locator('#content-academy')).toBeVisible({ timeout: 6000 });
  });
  ...
  test('[ACAD-07] אקדמיה — מעקב התקדמות עובד', async ({ page }) => {
    ...
    await goToTab(page, 'academy');
    await page.waitForTimeout(1500);
    const progressEl = page.locator('.progress-bar, [id*="progress"], .employee-progress, #admin-assignments-list, #my-assignments-list').first();
    const hasProgress = await progressEl.isVisible({ timeout: 8000 }).catch(() => false);
    if (!hasProgress) console.warn('[ACAD-07] מד התקדמות לא נמצא');
    await expect(page.locator('#content-academy')).toBeVisible({ timeout: 8000 });
  });
});
...
test.describe('צריכת תוכן (ACAD-08..10)', () => {
  ...
  test('[ACAD-08] אקדמיה — עובד צופה בהכשרה', async ({ page }) => {
    ...
    await page.waitForTimeout(1500);
    await expect(page.locator('#content-academy')).toBeVisible({ timeout: 8000 });
    const trainingItem = page.locator('.academy-item, .course-card, .training-item, #my-assignments-list .quiz-card, #library-list .quiz-card').first();
    const hasItem = await trainingItem.isVisible({ timeout: 5000 }).catch(() => false);
    if (hasItem) {
      await trainingItem.click();
      await page.waitForTimeout(600);
    } else {
      console.warn('[ACAD-08] פריט הכשרה לא נמצא לחיצה');
    }
  });
  ...
  test('[ACAD-09] אקדמיה — סימון הכשרה כהושלמה', async ({ page }) => {
    ...
    await page.waitForTimeout(1500);
    await expect(page.locator('#content-academy')).toBeVisible({ timeout: 8000 });
    const completeBtn = page.locator('button:has-text("סיימתי"), button:has-text("הושלם"), .complete-training-btn').first();
    const hasComplete = await completeBtn.isVisible({ timeout: 5000 }).catch(() => false);
    if (!hasComplete) console.warn('[ACAD-09] כפתור סיום הכשרה לא נמצא');
  });
  ...
  test('[ACAD-10] אקדמיה — תוכן וידאו / קובץ', async ({ page }) => {
    ...
    await page.waitForTimeout(1500);
    await expect(page.locator('#content-academy')).toBeVisible({ timeout: 8000 });
    const mediaEl = page.locator('video, iframe[src*="youtube"], .pdf-viewer, a[href*=".pdf"]').first();
    const hasMedia = await mediaEl.isVisible({ timeout: 5000 }).catch(() => false);
    if (!hasMedia) console.warn('[ACAD-10] תוכן מדיה לא נמצא');
  });
});
