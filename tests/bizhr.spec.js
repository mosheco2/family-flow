...
test('[BIZ-18] עובדים — הוספת עובד חדש למערכת', async ({ page }) => {
  ...
  await expect(
    page.locator('#invite-modal, [id*="invite"][id*="modal"], [id*="member"][id*="modal"]').first()
...
test('[BIZ-19] עובדים — עריכת פרטי עובד קיים', async ({ page }) => {
  ...
  await expect(memberItem).toBeVisible({ timeout: 6000 });
  ...
  await page.waitForTimeout(600);
  await expect(
    page.locator('[id*="edit"][id*="modal"], [id*="member"][id*="modal"]').first()
  ).toBeVisible({ timeout: 5000 });
});
...
test('[BIZ-22] משכורות — עיבוד יום משכורת לפי שעות', async ({ page }) => {
  ...
  await page.waitForTimeout(1200);
  const payrollInBank = page.locator('button:has-text("משכורות"), button:has-text("יום משכורת"), button:has-text("שכר"), #btn-payroll').first();
  const hasBankPayroll = await payrollInBank.isVisible({ timeout: 6000 }).catch(() => false);
  if (hasBankPayroll) {
    await expect(payrollInBank).toBeVisible({ timeout: 6000 });
  } else {
    await goToTab(page, 'members');
    await page.waitForTimeout(800);
    await expect(
      page.locator('button:has-text("משכורות"), button:has-text("עבד יום משכורת")').first()
    ).toBeVisible({ timeout: 5000 });
  }
});
