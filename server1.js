...
=== FLOW REWARDS (₣) — מנגנון מטבעות קהילתי ===
₣ הוא המטבע הקהילתי של המערכת. הוא שונה לחלוטין מ-₪ (שכר עובדים, הכנסות, הוצאות).

**כיצד לקוח מרוויח ₣ (ומה זה אומר לעסק):**
- לקוח מממש מבצע שהעסק פרסם בקהילה → הלקוח מקבל +3₣
- לקוח רוכש חבילה קהילתית שהעסק שותף בה → הלקוח מקבל +18₣
- לקוח מותיר ביקורת חיובית (4-5⭐) על העסק → הלקוח מקבל +7₣, העסק מקבל +8₣

**כיצד העסק מרוויח ₣:**
- ביקורת חיובית (4-5⭐) מלקוח על העסק: +8₣ לארנק העסק
- קבלת ליד/לקוח פוטנציאלי דרך הקהילה
- הצטרפות לקהילה ואישורה

**ארנק FLOW של העסק** (כפתור "⚡ ארנק FLOW" בלשונית קהילה):
- צפייה ביתרת ₣ הצבורים
- **אימות קוד הנחה של לקוח**: לקוח שמימש ₣ קיבל קוד FL... → הצג לעובד → עובד מזין את הקוד בארנק → הנחה מחושבת אוטומטית ב-₪
- 100₣ = ₪10 הנחה (שיעור ברירת המחדל)

**חשוב**: ₣ FLOW הוא מנגנון שיווק קהילתי — לא קשור לשכר עובדים, הכנסות, או ניהול פנים-עסקי.
...
app.post('/api/family/chat-assistant', async (req, res) => {
    try {
        ...
        if (isDirectFlowQuestion) {
            return res.json({
                success: true,
                answer: `**מטבעות FLOW ₣** הם המטבע הקהילתי של מערכת Oneflow Life — אמצעי תגמול בין משפחות לעסקים בקהילה. הם **שונים לחלוטין** מ-₪ שקלים פנים-משפחתיים.\n\n**כיצד צוברים ₣ FLOW** — רק דרך לשונית **קהילה** ⚡:\n• מימוש מבצע קהילתי ← **+3₣**\n• רכישת חבילה קהילתית ← **+18₣**\n• ביקורת חיובית 4-5⭐ על עסק בקהילה ← **+7₣**\n• הפניית משפחה חדשה לקהילה (קוד הפניה) ← **+35₣**\n• הצטרפות לקהילה חדשה ← בונוס קטן\n• כניסה יומית (אוטומטי) ← בונוס קטן\n\n**היתרה שלך:** לשונית **קהילה** → כפתור **⚡ ארנק FLOW**\n**מימוש:** מינימום 100₣ → קוד הנחה FL... → הצג לעסק\n\n⚠️ **חשוב:** מטבעות FLOW **אינם** נצברים ממשימות משפחתיות! משימות משפחה מתגמלות ב-₪ שקלים בלשונית **בנק** — מערכת נפרדת לחלוטין.`,
                action_type: 'OPEN_TAB',
                action_data: { tab: 'community' }
            });
        }
...
║  ⚠️  CRITICAL — READ BEFORE ANYTHING ELSE  ⚠️           ║
║                                                          ║
║  There are TWO completely separate currency systems.     ║
║  CONFUSING THEM IS YOUR #1 FORBIDDEN MISTAKE.           ║
║                                                          ║
║  ₪ SHEKELS  = family tasks reward, family bank tab       ║
║  ₣ FLOW     = community currency, community tab ONLY     ║
║                                                          ║
║  When user asks about "מטבעות" / "FLOW" / "₣":          ║
║  → Answer ONLY about ₣ FLOW earned in community tab.    ║
║  → NEVER say "complete tasks to earn מטבעות".           ║
║  → Tasks give ₪ shekels (family bank). NOT מטבעות FLOW. ║
╚══════════════════════════════════════════════════════════╝

=== TWO CURRENCY SYSTEMS — חייב להבחין ביניהם בכל תשובה ===

**מטבע 1: ₪ שקלים — בנק משפחתי פנימי**
- זהו כסף וירטואלי פנימי בין בני המשפחה בלבד
- כיצד מרוויחים ₪: ביצוע משימות משפחתיות עם תגמול, דמי כיס מהורים, ריבית
- איפה רואים: לשונית "בנק" (bank)
- שם נכון: "שקלים", "יתרה", "תגמול" — לא "מטבעות" ולא "₣"

**מטבע 2: ₣ FLOW — מטבע קהילתי (לא פנימי!)**
- זהו המטבע הקהילתי בין משפחות ועסקים בקהילה — לא קשור לתוך המשפחה
- כיצד מרוויחים ₣ (רק דרך לשונית "קהילה"):
  • מימוש מבצע קהילתי: +3₣
  • רכישת חבילה קהילתית: +18₣
  • ביקורת חיובית (4-5⭐) על עסק בקהילה: +7₣
  • הפניית משפחה לקהילה (קוד הפניה): +35₣
  • הצטרפות לקהילה חדשה: בונוס קטן
  • כניסה יומית (אוטומטי): בונוס קטן
- איפה רואים: לשונית "קהילה" → כפתור "ארנק FLOW ⚡"
- כיצד ממשים: ארנק FLOW → בחר כמות (מינימום 100₣) → קוד FL... → הצג לעסק

⛔ FORBIDDEN: "כדי לצבור מטבעות בצע משימות" — זה שגוי לחלוטין
⛔ FORBIDDEN: לקרוא לתגמול משימות "מטבעות" — משימות נותנות ₪ שקלים
✅ CORRECT: כשמישהו שואל "איך לצבור מטבעות?" — עני רק על ₣ FLOW דרך הקהילה

דוגמה לתשובה נכונה:
שאלה: "איך אוסף מטבעות?"
תשובה נכונה: "מטבעות FLOW ₣ נצברים דרך לשונית **קהילה**: מימוש מבצע (+3₣), רכישת חבילה (+18₣), ביקורת חיובית (+7₣), הפניית חבר (+35₣). לצפייה ביתרה: קהילה → ארנק FLOW ⚡"
תשובה שגויה: "מטבעות נצברים על ידי ביצוע משימות משפחתיות" ← זה אסור!
...
- bank → בנק משפחתי (יתרות ₪ שקלים, דמי כיס, ריבית לילדים — לא ₣)
- cashflow → תזרים (הכנסות/הוצאות)
- academy → אקדמיה (אתגרי לימוד לילדים עם תמריצים)
- tasks → משימות (תגמול ₪ שקלים לבנק המשפחה — לא ₣ FLOW!)
- community → קהילה מקומית (כאן בלבד רצים ₣ FLOW — ראה פירוט)
