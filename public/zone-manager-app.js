...
async function openCommunityDetail(commId, encodedName) {
    ...
    body.innerHTML = '<div class="text-center text-slate-400 py-8"><i class="fa-solid fa-spinner fa-spin mr-2"></i>טוען נתוני קהילה...</div>';
    ['families','businesses','promos','flow'].forEach(t => {
        document.getElementById(`zm-cdtab-${t}`).className = 'zm-cdtab text-xs font-bold px-3 py-1.5 rounded-t-lg border-b-2 border-transparent text-slate-500 hover:text-slate-700';
    });
    document.getElementById('zm-cdtab-families').className = 'zm-cdtab text-xs font-bold px-3 py-1.5 rounded-t-lg border-b-2 border-indigo-500 text-indigo-600 bg-indigo-50';
    _cdCurrentTab = 'families';
    modal.classList.remove('hidden');
    ...
    try {
        ...
        document.getElementById('zm-cd-meta').textContent = `${c.city || ''} • ${c.zone_name || ''} • ${c.status === 'active' ? 'פעיל' : c.status}`;
...
function switchCDTab(tab) {
    ...
    const c = colors[tab];
    document.getElementById(`zm-cdtab-${tab}`).className = `zm-cdtab text-xs font-bold px-3 py-1.5 rounded-t-lg border-b-2 border-${c}-500 text-${c}-600 bg-${c}-50`;
    renderCDTab(tab);
}
    ...
    if (tab === 'families') {
        ...
        <div class="flex justify-between items-center p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
            ...
            <div class="text-right">
                ...
                <p class="text-xs text-slate-500">${b.city || ''} ${b.phone ? '• ' + b.phone : ''}</p>
            </div>
        </div>`).join('');
    }
        ...
        <div class="p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
            ...
            ${p.expires_at ? `<p class="text-[10px] text-slate-400 text-left mt-1">תוקף עד: ${new Date(p.expires_at).toLocaleDateString('he-IL')}</p>` : ''}
        </div>`).join('');
    }
        ...
        body.innerHTML = `
        <div class="bg-amber-50 border border-amber-100 rounded-2xl p-4 text-center mb-4">
        ...
        <div class="flex justify-between items-center p-2.5 bg-white border border-slate-100 rounded-xl text-xs">
            ...
            <div class="text-right flex items-center gap-2">
                <span class="text-slate-700 font-medium">${actionLabels[t.action_type] || t.action_type}</span>
