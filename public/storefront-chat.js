 ...
 * אין FAB צף, אין כפתור בהירואין — רק בתוך "הפעילות שלי".
 *
...
(function () {
  ...
  function _injectCss() {
    ...
    s.textContent = [
      ...
      '.sc-msg.business{background:#fff;color:#1e293b;align-self:flex-end;border:1px solid #e2e8f0;border-bottom-right-radius:4px;box-shadow:0 1px 3px rgba(0,0,0,.06)}',
      '.sc-msg-time{font-size:10px;opacity:.55;margin-top:3px}',
      '#sc-chat-footer{padding:12px 14px;border-top:1px solid #e2e8f0;display:flex;gap:10px;background:#fff;flex-shrink:0}',
  ...
  // ── חלון צ'אט (overlay שקוף מאחורי activity panel) ─────────
  ...
  // ── הצגת/הסתרת כפתור צ'אט בפאנל האישי ───────────────────
  ...
  // ── פתיחת שיחה מה-API ────────────────────────────────────────
  async function _openChatSession() {
    var gid = _groupId();
    if (!gid) return;
    try {
  ...
  window.openCustomerChat = function() {
    // סגור את פאנל הפעילות כדי שחלון הצ'אט יהיה גלוי
  ...
  function _init() {
    ...
    // כאשר המשתמש מתחבר/מתנתק — עדכן את הכפתור
    // sc-auth.js קורא _updateHeaderBtn לאחר שינוי מצב התחברות
    var _origUpdate = window.scAuth && window.scAuth._updateHeaderBtn;
    if (window.scAuth) {
    ...
    // גיבוי: האזן לפתיחת פאנל הפעילות ועדכן שם
    var panel = document.getElementById('sc-activity-panel');
    if (panel) {
