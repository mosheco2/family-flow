...
window.onload = async () => { 
    ...
    if (inviteCode) { 
        ...
        hidePreloaderAndShowAuth('join'); return; 
    }
...
async function loadSAData() {
    ...
    try {
        ...
        const groupsList = document.getElementById('sa-groups-list'); let gHtml = '';
        data.groups.forEach(g => {
            let uHtml = data.users.filter(u => u.group_id === g.id).map(u => `<div class="flex justify-between items-center bg-white p-2 mt-1 rounded border text-sm"><span>${u.nickname} (${u.role})</span><button onclick="saDeleteUser(${u.id})" class="text-red-500 hover:text-red-700"><i class="fa-solid fa-trash"></i></button></div>`).join('');
...
function startAdminTour() {
    ...
    intro.setOptions({
        ...
        steps: [
            ...
            { element: '#tab-pantry', title: "מזווה ומלאי 📦", intro: "מעקב אחרי מלאי ביתי.", position: 'bottom' },
            { element: '#tab-bank', title: "ניהול הבנק 🏦", intro: "חלוקת דמי כיס וריביות לילדים.", position: 'bottom' },
            { element: '#tab-tasks', title: "משימות בית ✅", intro: "הגדירו משימות ותמחרו אותן.", position: 'bottom' },
            { element: '#tab-academy', title: "אקדמיה פיננסית 🎓", intro: "מבחנים מותאמים אישית בעזרת AI.", position: 'bottom' },
            { element: '#tab-budget', title: "תקציב 📊", intro: "מעקב הוצאות מול יעדים.", position: 'bottom' },
            { element: '#tab-recipes', title: "שף AI 👨‍🍳", intro: "מתכונים מותאמים למוצרים שיש בבית!", position: 'bottom' },
            { element: '#tab-members', title: "הזמנת המשפחה 👨‍👩‍👧‍👦", intro: "הזמנה בוואטסאפ לילדים ולמשפחה.", position: 'bottom' }
        ]
    });
...
function startChildTour() {
    ...
    intro.setOptions({
        ...
        steps: [
            ...
            { element: '#tab-bank', title: "הבנק והיעדים 🏦", intro: "תנאי דמי הכיס וקופות חיסכון.", position: 'bottom' },
            { element: '#tab-tasks', title: "משימות ותגמולים ✅", intro: "סיימו משימות וקבלו תגמול.", position: 'bottom' },
            { element: '#tab-academy', title: "האקדמיה הפיננסית 🎓", intro: "אתגרי למידה שווים.", position: 'bottom' },
            { element: '#tab-budget', title: "מעקב תקציב 📊", intro: "מעקב אחרי ההוצאות שלך.", position: 'bottom' },
            { element: '#tab-recipes', title: "שף AI 👨‍🍳", intro: "מתכונים מהדברים שיש בבית!", position: 'bottom' }
        ]
    });
    intro.onbeforechange(function(targetElement) { 
        if(!targetElement) return; const id = targetElement.id;
...
function buildAndRenderFeed() {
    ...
    if (currentGroup && currentGroup.created_at) {
        feedCache.push({
            ...
            user_id: 0,
            user_name: 'מערכת',
            date: new Date(currentGroup.created_at),
            title: 'הבנק המשפחתי נפתח בהצלחה! 🎉',
            amount: 0,
            status: 'welcome'
        });
    }
...
function renderRecipePantrySelection() {
    ...
    pantryCache.forEach(p => {
        html += `
        <label class="recipe-pantry-item-label flex items-center gap-1.5 bg-white border border-slate-200 px-2.5 py-1.5 rounded-lg cursor-pointer hover:border-orange-300 transition shadow-sm">
            <input type="checkbox" value="${p.item_name}" checked class="recipe-pantry-checkbox w-3.5 h-3.5 accent-orange-500">
            <span class="text-xs text-slate-700 font-medium">${p.item_name} <span class="text-[10px] text-slate-400">(${p.quantity})</span></span>
        </label>
        `;
    });
...
function selectAllRecipePantry() {
    ...
    document.querySelectorAll('.recipe-pantry-checkbox').forEach(cb => {
        cb.checked = isAllRecipePantrySelected;
    });
}
