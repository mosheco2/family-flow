...
async function loadDashboard() {
    ...
    try {
        ...
        console.error('Error in initial load');
    } finally {
        const preloader = document.getElementById('app-preloader'); 
...
window.submitBalanceAdjustment = async function() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showToast('success', 'היתרה עודכנה בהצלחה!');
        ...
        try {
            ...
            if (a && currentUser.role === 'ADMIN') { 
                ...
                if(children.length === 0) a.innerHTML = '<p class="text-center text-slate-400 text-sm py-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">אין ילדים רשומים במשפחה עדיין.</p>';
                    ...
                    const initial = m.nickname ? m.nickname.charAt(0).toUpperCase() : '?'; 
                    a.innerHTML += `<div class="bg-white rounded-2xl p-4 shadow-sm border border-slate-50 flex justify-between items-center mb-2">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center font-bold text-lg">${initial}</div>
                            <div>
                                <h4 class="font-bold text-slate-800 text-sm">${m.nickname || 'ילד'}</h4>
                                <p class="text-[10px] text-slate-400">₪${m.allowance_amount || 0}/שבוע • ${m.interest_rate || 0}% ריבית</p>
                                <p class="text-xs font-bold text-slate-700 mt-1">יתרה: <span class="text-blue-600">₪${m.balance || 0}</span></p> 
                            </div>
                        </div>
                        <div class="flex gap-2">
                            <button onclick="openBalanceAdjustmentModal(${m.id}, '${m.nickname}')" class="w-8 h-8 rounded-full bg-blue-50 hover:bg-blue-100 text-blue-500 flex items-center justify-center transition" title="קנס/בונוס"><i class="fa-solid fa-money-bill-transfer text-sm"></i></button>
                            ...
                            <button onclick="deleteUser(${m.id}, '${m.nickname}')" class="w-8 h-8 rounded-full bg-red-50 hover:bg-red-100 text-red-500 flex items-center justify-center transition"><i class="fa-solid fa-trash text-sm"></i></button>
                        </div>
                    </div>`; 
                }); 
            } 
...
// ----------------------------------------------------
// פונקציה מרכזית לטעינת נתונים - מוגנת ומוקשחת מפני קריסות
// ----------------------------------------------------
async function fetchData() {
    ...
    try {
        ...
        data = await res.json();
    } catch (e) { return; } // מונע קריסה במקרה של בעיית אינטרנט
    ...
    } catch(e) { console.error('Core data sync error', e); }
    
    try { allTasks = Array.isArray(data.tasks) ? data.tasks : []; } catch(e){}
    try { bundlesCache = Array.isArray(data.quiz_bundles) ? data.quiz_bundles : []; } catch(e){}
    try { pantryCache = Array.isArray(data.pantry) ? data.pantry : []; } catch(e){}
    try { if (data.all_bundles && data.all_bundles.length > 0) allBundles = data.all_bundles; } catch(e){}

    try { if (currentUser.role === 'ADMIN') renderAdminAcademy(); else { renderMyAssignments(bundlesCache); renderLibrary(); } } catch(e) {}
    try { renderTasks(allTasks); } catch(e) {}
    try { renderPantry(); renderRecipePantrySelection(); } catch(e) {}
    ...
    } catch(e) { console.error('Transactions fetch error:', e); allTransactions = []; }
...
function renderShopList() {
    ...
    if (isShopTabActive) {
        const f = document.getElementById('cart-footer'); if(f) f.classList.remove('hidden'); 
        const fc = document.getElementById('fab-container'); if(fc) fc.classList.add('fab-lifted');
    } else {
        const f = document.getElementById('cart-footer'); if(f) f.classList.add('hidden'); 
        const fc = document.getElementById('fab-container'); if(fc) fc.classList.remove('fab-lifted');
    }
...
// ============================================================
// --- SA Premium Toggle (Continued Below) ---
// ============================================================
...
// --- LOCAL FORECAST CALCULATION ENGINE ---
function renderForecast() {
    ...
    if (currentForecastMode === 'monthly') {
        if(!periodVal) {
            ...
            endDate = new Date(parts[0], parts[1], 0, 23, 59, 59);
        }
    } else {
    ...
    relevantTxs.forEach(t => {
        ...
        // תיקון זיהוי בוליאני של מסד נתונים
        const isRec = t.is_recurring === true || String(t.is_recurring) === 'true';

        if (!isRec) {
            // פעולה חד פעמית (כולל קניות)
            if (txDate >= startDate && txDate <= endDate) {
                items.push({
                    ...
                    sort_date: txDate.getTime()
                });
                if (isInc) periodIncome += amt; else periodExpense += amt;
                
                // התיקון: קיזוז עתידי יתבצע *רק* אם הפעולה עוד לא קרתה במציאות
                if (txDate > now && txDate <= endDate) {
                    if (isInc) futureNetChange += amt; else futureNetChange -= amt;
                }
            }
        } else {
            // פעולה קבועה - בניית המופעים העתידיים שלה
            let virtualDate = new Date(txDate.getFullYear(), txDate.getMonth(), txDate.getDate());
            
            let occurrences = 0;
            while (virtualDate <= endDate) {
                if (t.end_month) {
                    const endParts = t.end_month.split('-');
                    const endM = new Date(endParts[0], endParts[1] - 1, 1);
                    endM.setMonth(endM.getMonth() + 1); // עד סוף החודש הנבחר
                    if (virtualDate >= endM) break;
                }

                // סופרים מופעים רק ממועד ההתחלה והלאה
                if (virtualDate >= txDate) {
                    const isVirtualInPeriod = virtualDate >= startDate && virtualDate <= endDate;
                    
                    if (isVirtualInPeriod) {
                        items.push({
                            id: `${t.id}_v_${occurrences}`, type: t.type, amount: amt, category: t.category, 
                            description: t.description, is_recurring: true, user_name: t.user_name, 
                            date_str: `קבוע (${virtualDate.toLocaleDateString('he-IL')})`,
                            sort_date: virtualDate.getTime()
                        });
                        if (isInc) periodIncome += amt; else periodExpense += amt;
                    }

                    // התיקון: קיזוז עתידי ליתרה יקרה *רק* למופעים שעוד לא התרחשו
                ...
                occurrences++;
                if (occurrences > 120) break; // הגנת שפיות מפני לולאה אינסופית
            }
        }
    });
...
async function submitBankSettings() { 
    ...
    await fetch(`${API}/admin/update-settings`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ userId: uid, allowance, interest }) }); 
...
async function triggerPayday() { 
    ...
    try { 
        const res = await fetch(`${API}/admin/payday`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ groupId: currentGroup.id }) }); 
        const data = await res.json(); 
        if(data.success) { triggerConfetti(); showToast('success', `חולקו ${data.totalDistributed} ש"ח בהצלחה!`); fetchData(); } 
        else { showToast('error', data.error); } 
    } catch(e) { showToast('error', 'שגיאה'); } 
}

function openGoalModal() { 
    if(currentUser.role === 'ADMIN') { document.getElementById('goal-user-select-container').classList.remove('hidden'); } 
    document.getElementById('goal-title').value = ''; document.getElementById('goal-target').value = ''; document.getElementById('goal-modal').classList.remove('hidden'); 
}
...
async function submitGoal() { 
    ...
    const targetUserId = (currentUser.role === 'ADMIN' && document.getElementById('goal-user-select-container').style.display !== 'none') ? select.value : null; 
    await fetch(`${API}/goals`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ userId: currentUser.id, targetUserId, title, target }) }); 
    triggerConfetti(); 
...
async function submitDeposit() { 
    ...
    const res = await fetch(`${API}/goals/deposit`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ userId: currentUser.id, goalId, amount }) }); 
    const data = await res.json(); 
    if (data.success) { triggerConfetti(); document.getElementById('goal-deposit-modal').classList.add('hidden'); fetchData(); } 
    else showToast('error', data.error); 
}
...
window.toggleTransType = function(type) {
    ...
    document.getElementById('btn-trans-onetime').className = isRecurring ? 'flex-1 py-1.5 text-sm font-bold bg-white text-blue-600 rounded-lg shadow-sm transition' : 'flex-1 py-1.5 text-sm font-bold text-slate-500 hover:text-slate-700 rounded-lg transition';
...
async function submitLoan() { 
    await fetch(`${API}/loans/request`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({userId:currentUser.id, amount:val('loan-amount'), reason:val('loan-reason')})}); 
    document.getElementById('loan-modal').classList.add('hidden'); 
    showToast('success', 'בקשת ההלוואה נשלחה להורה 📨'); 
    fetchData(); 
    fetchLoans(); 
}
