...
async function updateSACredentials() {
    ...
    if(!confirm('האם לשנות את פרטי המנהל הראשי?')) return;
    try {
...
async function clearEntireCart() {
    if(!confirm('האם למחוק את כל הפריטים (הפעילים והחסרים) מהרשימה? פעולה זו אינה הפיכה.')) return;
