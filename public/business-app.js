...
// תבנית ה-HTML ל-PDF - גרסה 1.1.4 (הסרת הדבקת מילים, סידור עמודת יחידות וסימני פיסוק עם rlm)
// תבנית ה-HTML ל-PDF - גרסה 1.1.5 (תיקון רווחים קשיחים וסימני פיסוק אחרונים)
function getOrderHtmlTemplate(orderInfo) {
    ...
    if (itemsArr.length === 0) {
        ...
        itemsHtml = itemsArr.map((i, index) => `
            <tr style="background-color: ${index % 2 === 0 ? '#ffffff' : '#f8fafc'}; border-bottom: 1px solid #cbd5e1; page-break-inside: avoid;" dir="rtl">
                ...
                <td style="padding: 12px 8px; text-align: right; border: 1px solid #cbd5e1; font-weight: bold; color: #1e293b; unicode-bidi: isolate;" dir="rtl">
                    ${safeStr(i.name)}
                </td>
                <td style="padding: 12px 8px; text-align: center; border: 1px solid #cbd5e1; font-weight: bold; color: #4f46e5;" dir="rtl">
                    <div style="display: inline-flex; align-items: center; justify-content: center; direction: rtl; unicode-bidi: isolate;">
    ...
    // פונקציה לטיפול בכותרות ונקודתיים בעברית:
    // מחליף את הרווח לרווח קשיח (כדי שהמילה עצמה לא תרד שורה ותידבק) ושם rlm בסוף לשמור על הנקודתיים מימין. 
    // בנוסף, שמתי unicode-bidi: isolate כדי שהאנגלית שאחרי זה לא תמשוך את הנקודתיים אליה.
    const fixLabel = (label) => `<span style="font-weight: bold; color: #334155; unicode-bidi: isolate;">${label.replace(/ /g, '&nbsp;')}&rlm;</span>`;

    const customerNumHtml = orderInfo.customerNumber ? `<div style="margin-bottom: 8px; text-align: right;" dir="rtl">${fixLabel('מספר לקוח:')} <span dir="ltr" style="background-color: #eef2ff; padding: 4px 10px; border-radius: 6px; color: #4f46e5; font-weight: bold; unicode-bidi: isolate;">${safeStr(orderInfo.customerNumber)}</span></div>` : '';
    const branchHtml = orderInfo.branchName ? `<div style="margin-bottom: 8px; text-align: right;" dir="rtl">${fixLabel('עבור סניף/מחלקה:')} <span style="unicode-bidi: isolate;">${safeStr(orderInfo.branchName)}&rlm;</span></div>` : '';
    const phoneHtml = orderInfo.supplierPhone ? `<div style="margin-bottom: 8px; text-align: right;" dir="rtl">${fixLabel('טלפון:')} <span style="unicode-bidi: isolate;" dir="ltr">${safeStr(orderInfo.supplierPhone)}</span></div>` : '';
    const emailHtml = orderInfo.supplierEmail ? `<div style="margin-bottom: 8px; text-align: right;" dir="rtl">${fixLabel('דוא"ל:')} <span style="unicode-bidi: isolate;" dir="ltr">${safeStr(orderInfo.supplierEmail)}</span></div>` : '';
    ...
    return `
        <div style="direction: rtl; font-family: Arial, sans-serif; color: #1e293b; background: white; width: 1040px; box-sizing: border-box; padding: 25px; text-align: right;" dir="rtl">
                ...
                <tr>
                    <td style="vertical-align: middle; text-align: right; width: 60%;" dir="rtl">
                        ...
                        <div style="font-size: 16px; color: #0f172a; background: #f8fafc; display: inline-block; padding: 8px 18px; border-radius: 8px; border: 1px solid #e2e8f0;">
                            ${fixLabel('מספר הזמנה:')} <b dir="ltr" style="color: #4f46e5; unicode-bidi: isolate;">#${orderInfo.orderId || 'חדש'}</b>
                        </div>
                    </td>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px;" dir="rtl">
                <tr>
                    <td style="width: 48%; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; vertical-align: top; text-align: right;" dir="rtl">
                        ...
                        <div style="margin-bottom: 9px; text-align: right;" dir="rtl">${fixLabel('שם העסק:')} <span style="unicode-bidi: isolate;">${safeStr(currentGroup.name)}&rlm;</span></div>
                        <div style="margin-bottom: 9px; text-align: right;" dir="rtl">${fixLabel('איש קשר:')} <span style="unicode-bidi: isolate;">${safeStr(currentUser.nickname)}&rlm;</span></div>
                        ${branchHtml}
                        ${customerNumHtml}
                        <div style="margin-top: 16px; font-size: 13px; color: #64748b; text-align: right;" dir="rtl">${fixLabel('הופק ב:')} <span dir="ltr" style="unicode-bidi: isolate;">${new Date().toLocaleDateString('he-IL')} ${new Date().toLocaleTimeString('he-IL', {hour:'2-digit', minute:'2-digit'})}</span></div>
                    </td>
                    <td style="width: 4%;"></td>
                    <td style="width: 48%; background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; vertical-align: top; text-align: right;" dir="rtl">
                        ...
                        <div style="margin-bottom: 9px; text-align: right;" dir="rtl">${fixLabel('לכבוד:')} <span style="unicode-bidi: isolate;">${safeStr(orderInfo.supplierName)}&rlm;</span></div>
                        ${phoneHtml}
                        ${emailHtml}
                    </td>
                </tr>
            ...
            <div style="margin-bottom: 25px; font-size: 15px; line-height: 1.7; color: #334155; text-align: right;" dir="rtl">
                ...
                <span>מצ"ב פירוט הזמנת רכש מאושרת ממערכת ההזמנות שלנו.&rlm; נא לספק את הסחורה המפורטת מטה בהקדם האפשרי ולפי תנאי הסחר והמחירון&nbsp;שסוכמו.&rlm;</span>
            </div>
            ...
            <div style="border-top: 3px solid #cbd5e1; padding-top: 20px; page-break-inside: avoid; text-align: right;" dir="rtl">
                <h2 style="margin: 0; font-size: 22px; color: #0f172a;">סה"כ&nbsp;לתשלום&nbsp;משוער:&rlm; <span dir="ltr" style="color: #4f46e5; unicode-bidi: isolate;">₪${(orderInfo.totalAmount || orderInfo.total || 0).toFixed(2)}</span></h2>
...
(function addVersionBadge() {
    if (!document.getElementById('oneflow-version-badge')) {
        ...
        badge.innerHTML = 'גרסה 1.1.5 (תיקון רווחים טבעיים וסימני פיסוק עם rlm)';
