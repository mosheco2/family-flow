...
async function saveAllBanners() {
    ...
    const topText = safeVal('sa-banner-top-text'); const topLink = safeVal('sa-banner-top-link'); const topImg = safeVal('sa-banner-top-img');
...
async function checkTimeclockStatus() {
    try {
        ...
        const btn = document.getElementById('btn-punch'); const icon = document.getElementById('tc-icon'); const text = document.getElementById('tc-btn-text'); const info = document.getElementById('tc-active-info'); const startTime = document.getElementById('tc-start-time');
        if(!btn) return;
...
async function fetchTimeclockReport() {
    try {
        ...
        if(filterEl && filterEl.options.length === 0) { filterEl.innerHTML = '<option value="all">כלל העובדים</option>'; membersCache.forEach(m => { if(m.role !== 'ADMIN') filterEl.innerHTML += `<option value="${m.id}">${m.nickname}</option>`; }); }
        ...
        const list = document.getElementById('timeclock-report-list'); if(!list) return;
...
async function submitTask() { 
    ...
    try {
        ...
        if (btn) { btn.disabled = false; btn.innerText = 'הקצה משימה'; }
    }
}
...
async function submitPantryItem() {
    ...
    try {
        ...
        if(btn) { btn.disabled = false; btn.innerText = 'הוסף למאגר'; }
    }
}
...
async function finishQuiz() {
    ...
    document.getElementById('quiz-icon').innerHTML = passed ? '🏆' : '📚'; document.getElementById('quiz-msg-title').innerText = passed ? 'כל הכבוד!' : 'לא נורא, אפשר לנסות שוב...'; document.getElementById('quiz-msg-desc').innerText = passed ? `השלמת את חפיפת הנהלים וזכית בתמריץ של ₪${currentQuizData.custom_reward || currentQuizData.default_reward}` : `יש להגיע לציון של ${currentQuizData.threshold}% כדי לעבור את ההכשרה.`; document.getElementById('quiz-score-display').innerText = `ציון סופי: ${finalScore}%`;
...
async function submitShopItem() { 
    ...
    try { 
        ...
        if (data.success) { 
            ...
            itemInput.value = ''; document.getElementById('shop-est-price').value = ''; 
            document.getElementById('shop-quantity').value = 1; document.getElementById('shop-unit').value = "יח'"; 
            document.getElementById('shop-upp').value = 1; document.getElementById('suggestions').classList.add('hidden'); 
            if (data.alert && data.id) wisdomCache[data.id] = data.alert.msg; 
            showToast('success', 'נוסף לרשימה'); 
            fetchData(); 
        } else {
            showToast('error', data.error || 'שגיאת שרת בהוספת פריט לרכש');
        }
    } catch(e) {
        showToast('error', 'שגיאת תקשורת מול השרת');
    } finally { 
