...
window.injectBusinessUI = function() {
    ...
    ['content-shifts', 'cust-main-tabs', 'content-sales', 'content-pos', 'pos-modifiers-modal', 'pos-tender-modal'].forEach(id => {
    ...
    if(contentShiftsNew) {
        contentShiftsNew.insertAdjacentHTML('afterend', `
            <div id="content-pos" class="hidden h-[85vh] flex flex-col md:flex-row gap-4 pb-20">
            ...
            <div id="content-sales" class="hidden pb-20">
    ...
    if(!getEl('tab-sales')) {
        ...
        if(tabBank) {
            tabBank.insertAdjacentHTML('beforebegin', `<button onclick="window.switchTab('pos')" id="tab-pos" class="tab-btn bg-gradient-to-r from-emerald-500 to-green-600 text-white border-transparent shadow-md font-black">קופה (POS) 💰</button>`);
...
if (!window.origSwitchTabWrapped) {
    ...
    window.switchTab = function(t) {
        if (t === 'pos') {
            ...
            const contentPos = document.getElementById('content-pos'); if (contentPos) contentPos.classList.remove('hidden');
            const tabPos = document.getElementById('tab-pos'); if(tabPos) tabPos.classList.add('tab-active');
            
            // הסתרת פוטר ותפריט צף אם קיימים
            const footer = document.getElementById('cart-footer'); if (footer) footer.classList.add('hidden');
            const fab = document.getElementById('fab-container'); if(fab) fab.classList.remove('fab-lifted');
            
            window.renderPOSCatalog('all');
        } else {
            const contentPos = document.getElementById('content-pos'); if (contentPos) contentPos.classList.add('hidden');
            const tabPos = document.getElementById('tab-pos'); if(tabPos) tabPos.classList.remove('tab-active');
            origSwitchTab(t);
        }
    };
    window.origSwitchTabWrapped = true;
}
...
window.finalizePOSOrder = async function() {
    ...
    try {
        ...
        if((await res.json()).success) {
            ...
            showToast('success', 'העסקה הושלמה!'); triggerConfetti(); window.posCart = []; window.renderPOSCart(); document.getElementById('pos-tender-modal').classList.add('hidden');
            if(typeof window.fetchStoreOrders === 'function') window.fetchStoreOrders();
