...
window.onload = async () => { 
    // --- מנגנון חיווי התעוררות שרת ---
    const wakeupTimer = setTimeout(() => {
        const pText = document.querySelector('#app-preloader p');
        if (pText) pText.innerText = 'השרת מתעורר משינה (עלול לקחת כדקה)... ⏳';
    }, 4000); // אחרי 4 שניות נעדכן את המשתמש שהשרת עולה
    ...
    if(saved) { 
        try { 
            ...
            if(session && session.user && session.user.id) { 
                ...
                clearTimeout(wakeupTimer); // השרת ענה - אפשר לבטל את חיווי ההתעוררות
                if(res.ok) { 
            ...
            clearTimeout(wakeupTimer);
            localStorage.removeItem('ofl_session');
            hidePreloaderAndShowAuth('login'); // שגיאת רשת/קריסה - נוריד מסך טעינה ונעביר להתחברות
        } 
    } else { 
        clearTimeout(wakeupTimer);
        hidePreloaderAndShowAuth('login');
...
function startAdminTour() {
    ...
    intro.setOptions({
        ...
        steps: [
            ...
            { element: '#tour-fab-btn', title: "הוספה מהירה ⚡", intro: "לחיצה על הפלוס כאן תאפשר לכם לרשום הוצאה או הכנסה מכל מסך באפליקציה." },
...
function startChildTour() {
    ...
    intro.setOptions({
        ...
        steps: [
            ...
            { element: '#user-balance', title: "הארנק שלי 💳", intro: "כאן תוכלו לראות בדיוק כמה כסף פנוי יש לכם עכשיו בחשבון." },
            ...
            { element: '#tab-budget', title: "מעקב תקציב 📊", intro: "כאן תוכלו לעקוב אחרי ההוצאות שלכם מול היעדים שהוגדרו ולשלוט בכסף שלכם." },
...
async function loadDashboard() {
    ...
    try {
        ...
        // מנגנון זה מבטיח שמסך הטעינה יירד גם אם בקשות הרשת נכשלו
        const preloader = document.getElementById('app-preloader'); 
