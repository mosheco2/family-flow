...
async function _lgDeleteGame(id, btnEl) {
  ...
  try {
    const r = await fetch(`/api/live-games/${id}`, { method: 'DELETE', headers: { Authorization: saToken } });
    const d = await r.json();
    if (d.success) _fetchLGList();
    else alert('שגיאה: ' + (d.error || 'לא ידועה'));
          ...
          <div>
            ...
            <div class="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-2xl p-5 text-white shadow-lg mb-5">
              <div class="flex justify-between items-start gap-3 flex-wrap">
                ...
                <button onclick="saKHSeedList()" id="sa-kh-seed-list-btn"
                  class="bg-white/20 hover:bg-white/30 text-white text-xs font-bold px-4 py-2 rounded-xl border border-white/30 transition whitespace-nowrap">
                ...
                <button onclick="saKHSeedEditorDraft()" id="sa-kh-editor-draft-btn"
...
async function saKHSeedList() {
    const btn = document.getElementById('sa-kh-seed-list-btn');
    if (!confirm('לזרוע 10 כתבות דוגמה לרשימה גנרית (תגית חינוך-ומשפחה)?')) return;
    if (btn) { btn.disabled = true; btn.textContent = '⏳ זורע...'; }
    try {
        const d = await fetch('/api/kol-haam/seed-list-sample', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ secret: 'SEED_DEMO_2026' })
        }).then(r => r.json());
        if (d.success) {
            showToast('success', `נזרעו ${d.ids.length} כתבות — תגית: ${d.tag}`);
        } else {
...
async function saKHSeedEditorDraft() {
    const btn = document.getElementById('sa-kh-editor-draft-btn');
    if (btn) { btn.disabled = true; btn.textContent = '⏳ יוצר...'; }
    try {
        const d = await fetch('/api/kol-haam/seed-editor-draft', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ secret: 'SEED_DEMO_2026' })
        }).then(r => r.json());
        if (d.success) {
            showToast('success', d.message || 'טיוטה נוצרה!');
            if (d.editorUrl) {
                setTimeout(() => {
                    if (confirm('טיוטה מוכנה! לפתוח את העורך עכשיו?')) {
                        window.open(d.editorUrl, '_blank');
                    }
                }, 500);
            }
        } else showToast('error', d.error || 'שגיאה');
...
const PRICING_CATALOG_DEFAULT = [
  ...
  },
  {
    ...
    modules: [
      { id: 'user_manager',  name: 'מנהל / בעל עסק', price: 0,  free: true,  desc: 'כלול בכל חבילה — תמיד חינם, ללא קשר לסוג העסק או החבילה' },
      { id: 'user_extra',    name: 'עובד נוסף',        price: 9,  free: false, desc: 'כל עובד נוסף ללא קשר לתפקידו, דרגתו או הרשאותיו — מחיר אחיד לנציג' },
    ]
  },
  {
...
async function renderPricingCatalogView() {
  ...
  const fmEl = document.getElementById('pricing-free-months-control');
  if (fmEl) {
    fmEl.innerHTML = `
      <div class="bg-green-50 border border-green-200 rounded-2xl p-4 mb-5 flex flex-wrap items-center gap-4">
        <div class="flex items-center gap-2">
          <span class="text-lg">🎁</span>
          <span class="font-bold text-green-800 text-sm">הטבת חינם לוויזארד</span>
        </div>
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" id="free-months-toggle" ${_wizFreeMonths > 0 ? 'checked' : ''}
            onchange="_wizFreeMonths = this.checked ? (parseInt(document.getElementById('free-months-count').value)||3) : 0; document.getElementById('free-months-count').disabled = !this.checked;"
            class="w-4 h-4 accent-green-600">
          <span class="text-sm font-bold text-green-700">פעיל</span>
        </label>
        <div class="flex items-center gap-2">
          <input type="number" id="free-months-count" min="1" max="12" value="${_wizFreeMonths || 3}"
            ${_wizFreeMonths === 0 ? 'disabled' : ''}
            oninput="_wizFreeMonths = this.checked || document.getElementById('free-months-toggle').checked ? (parseInt(this.value)||3) : 0"
            class="w-16 border border-green-300 rounded-lg px-2 py-1.5 text-center text-sm font-bold text-green-800 focus:border-green-500 focus:outline-none">
...
async function aiGenerateBusiness() {
  ...
  if (!name) return showSAToast?.('יש להזין שם עסק') || alert('יש להזין שם עסק');
  const btn = document.getElementById('aib-generate-btn');
  btn.disabled = true;
  btn.textContent = '⏳ מייצר...';
  document.getElementById('aib-step1').style.display = 'none';
  document.getElementById('aib-loading').style.display = 'flex';
  try {
    const body = {
      businessName: name,
      businessNameEn: document.getElementById('aib-name-en')?.value?.trim() || '',
    ...
    document.getElementById('aib-loading').style.display = 'none';
    document.getElementById('aib-step1').style.display = 'block';
    btn.disabled = false;
    btn.textContent = '✨ צור עסק עם AI';
    alert('שגיאה: ' + e.message);
  }
}
...
function renderAIBPreview() {
  ...
  document.getElementById('aib-prev-slogan-en').value = d.profile?.slogan_en || '';
  document.getElementById('aib-prev-welcome').value = d.profile?.welcome_message || '';
  document.getElementById('aib-prev-welcome-en').value = d.profile?.welcome_message_en || '';
...
function syncAIBFromForm() {
  ...
  _aiBuilderData.profile = {
    ...
    name_en: document.getElementById('aib-prev-name-en').value,
    slogan: document.getElementById('aib-prev-slogan').value,
    slogan_en: document.getElementById('aib-prev-slogan-en').value,
    welcome_message: document.getElementById('aib-prev-welcome').value,
    welcome_message_en: document.getElementById('aib-prev-welcome-en').value,
    accent_color: document.getElementById('aib-prev-color').value
  };
  _aiBuilderData.settings = {
    delivery_fee: parseFloat(document.getElementById('aib-prev-delivery').value) || 15,
    min_order: parseFloat(document.getElementById('aib-prev-minorder').value) || 50,
    delivery_eta_min: parseInt(document.getElementById('aib-prev-eta').value) || 35,
    open_time: document.getElementById('aib-prev-open').value || '09:00',
    close_time: document.getElementById('aib-prev-close').value || '23:00',
    free_delivery_above: parseFloat(document.getElementById('aib-prev-freedeliv').value) || 0
  };
}

function _escH(s) { return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }

function renderAIBCatalog() {
  const catalog = _aiBuilderData?.catalog || [];
  const el = document.getElementById('aib-catalog-body');
  if (!el) return;
  el.innerHTML = catalog.map((cat, ci) => `
    <div style="margin-bottom:20px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
        <input value="${_escH(cat.category)}" placeholder="קטגוריה (עברית)" onchange="aiCatChange(${ci},'category',this.value)" style="flex:1;padding:6px 10px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px">
        <input value="${_escH(cat.category_en||'')}" placeholder="Category (English)" onchange="aiCatChange(${ci},'category_en',this.value)" style="flex:1;padding:6px 10px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px">
        <button onclick="aiDeleteCat(${ci})" style="padding:6px 10px;border:1px solid #fca5a5;border-radius:8px;background:#fff;color:#ef4444;cursor:pointer;font-size:12px">🗑️</button>
      </div>
      ${(cat.products||[]).map((p,pi) => `
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px;margin-bottom:8px;display:grid;grid-template-columns:1fr 1fr 80px auto;gap:8px;align-items:start">
          <div>
            <input value="${_escH(p.name)}" placeholder="שם (עברית)" onchange="aiProdChange(${ci},${pi},'name',this.value)" style="width:100%;padding:5px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px;margin-bottom:4px">
            <textarea placeholder="תיאור (עברית)" onchange="aiProdChange(${ci},${pi},'description',this.value)" style="width:100%;padding:5px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:11px;resize:none;height:48px">${_escH(p.description||'')}</textarea>
          </div>
          <div>
    ...
    </div>`).join('') + `<button onclick="aiAddCat()" style="padding:8px 16px;border:1px dashed #6366f1;border-radius:8px;background:#fff;color:#6366f1;cursor:pointer;font-size:13px;margin-top:4px">+ הוסף קטגוריה</button>`;
}
...
function renderAIBImages() {
  const catalog = _aiBuilderData?.catalog || [];
  const el = document.getElementById('aib-images-body');
  if (!el) return;
  const rows = [];
...
async function aiLoadTemplates() {
  ...
  if (!el) return;
  el.innerHTML = d.success && d.templates.length
    ? d.templates.map(t => `<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px">
        <div style="flex:1"><div style="font-weight:600;font-size:13px">${_escH(t.name)}</div><div style="font-size:11px;color:#94a3b8">${t.business_type||''} · ${new Date(t.created_at).toLocaleDateString('he-IL')}</div></div>
        <button onclick="aiLoadTemplate(${t.id})" style="padding:5px 12px;border:1px solid #6366f1;border-radius:6px;background:#fff;color:#6366f1;cursor:pointer;font-size:12px">טען</button>
        <button onclick="aiDeleteTemplate(${t.id},this)" style="padding:5px 10px;border:1px solid #fca5a5;border-radius:6px;background:#fff;color:#ef4444;cursor:pointer;font-size:12px">🗑️</button>
      </div>`).join('')
    : '<div style="color:#94a3b8;font-size:13px;padding:12px">אין templates שמורים</div>';
}
...
function aiResetBuilder() {
  ...
  const prog = document.getElementById('aib-progress');
  const btn = document.getElementById('aib-generate-btn');
  if (s1) s1.style.display = 'block';
  if (prev) prev.style.display = 'none';
  if (succ) succ.style.display = 'none';
  if (prog) prog.style.display = 'none';
  if (btn) { btn.disabled = false; btn.textContent = '✨ צור עסק עם AI'; }
}
...
function initAIBuilder() {
  // nothing to initialize on first load — view is static HTML
}

async function aiGenerateBusiness() {
  const name = document.getElementById('aib-name')?.value?.trim();
  if (!name) return showToast('error', 'יש להזין שם עסק');
...
function renderAIBPreview() {
  ...
  document.getElementById('aib-prev-welcome-en').value = d.profile?.welcome_message_en || '';
  document.getElementById('aib-prev-color').value = d.profile?.accent_color || '#e63946';
  document.getElementById('aib-prev-color-swatch').style.background = d.profile?.accent_color || '#e63946';
  document.getElementById('aib-prev-delivery').value = d.settings?.delivery_fee ?? 15;
  document.getElementById('aib-prev-minorder').value = d.settings?.min_order ?? 50;
  document.getElementById('aib-prev-eta').value = d.settings?.delivery_eta_min ?? 35;
  document.getElementById('aib-prev-open').value = d.settings?.open_time || '09:00';
...
function renderAIBCatalog() {
  ...
  if (!el) return;
  el.innerHTML = catalog.map((cat, ci) => `
    <div class="aib-cat-block" data-ci="${ci}" style="margin-bottom:20px">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
        <input value="${escH(cat.category)}" placeholder="קטגוריה (עברית)" onchange="aiCatChange(${ci},'category',this.value)" style="flex:1;padding:6px 10px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px">
        <input value="${escH(cat.category_en||'')}" placeholder="Category (English)" onchange="aiCatChange(${ci},'category_en',this.value)" style="flex:1;padding:6px 10px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px">
        <button onclick="aiDeleteCat(${ci})" style="padding:6px 10px;border:1px solid #fca5a5;border-radius:8px;background:#fff;color:#ef4444;cursor:pointer;font-size:12px">🗑️</button>
      </div>
      ${(cat.products||[]).map((p,pi) => `
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px;margin-bottom:8px;display:grid;grid-template-columns:1fr 1fr 80px auto;gap:8px;align-items:start">
          <div>
            <input value="${escH(p.name)}" placeholder="שם (עברית)" onchange="aiProdChange(${ci},${pi},'name',this.value)" style="width:100%;padding:5px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px;margin-bottom:4px">
            <textarea placeholder="תיאור (עברית)" onchange="aiProdChange(${ci},${pi},'description',this.value)" style="width:100%;padding:5px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:11px;resize:none;height:48px">${escH(p.description||'')}</textarea>
          </div>
          <div>
            <input value="${escH(p.name_en||'')}" placeholder="Name (English)" onchange="aiProdChange(${ci},${pi},'name_en',this.value)" style="width:100%;padding:5px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px;margin-bottom:4px">
            <textarea placeholder="Description (English)" onchange="aiProdChange(${ci},${pi},'description_en',this.value)" style="width:100%;padding:5px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:11px;resize:none;height:48px">${escH(p.description_en||'')}</textarea>
          </div>
          <input type="number" value="${p.price||0}" min="0" step="0.5" onchange="aiProdChange(${ci},${pi},'price',parseFloat(this.value))" style="padding:5px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:13px;font-weight:700">
          <button onclick="aiDeleteProd(${ci},${pi})" style="padding:5px 8px;border:1px solid #fca5a5;border-radius:6px;background:#fff;color:#ef4444;cursor:pointer">🗑️</button>
        </div>`).join('')}
...
function renderAIBPromos() {
  ...
  el.innerHTML = promos.map((p,i) => `
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px;margin-bottom:8px;display:grid;grid-template-columns:1fr 1fr 80px 80px auto;gap:8px;align-items:center">
      <input value="${escH(p.title||'')}" placeholder="כותרת (עברית)" onchange="aiPromoChange(${i},'title',this.value)" style="padding:6px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px">
      <input value="${escH(p.title_en||'')}" placeholder="Title (English)" onchange="aiPromoChange(${i},'title_en',this.value)" style="padding:6px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px">
      <input type="number" value="${p.discount_pct||0}" min="0" max="100" placeholder="%" onchange="aiPromoChange(${i},'discount_pct',parseFloat(this.value))" style="padding:6px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:13px">
      <input type="number" value="${p.min_order||0}" min="0" placeholder="מינ׳" onchange="aiPromoChange(${i},'min_order',parseFloat(this.value))" style="padding:6px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:13px">
      <button onclick="aiDeletePromo(${i})" style="padding:6px 10px;border:1px solid #fca5a5;border-radius:6px;background:#fff;color:#ef4444;cursor:pointer">🗑️</button>
    </div>`).join('') + `<button onclick="aiAddPromo()" style="padding:8px 16px;border:1px dashed #6366f1;border-radius:8px;background:#fff;color:#6366f1;cursor:pointer;font-size:13px;margin-top:4px">+ הוסף מבצע</button>`;
...
function renderAIBImages() {
  ...
  catalog.forEach((cat, ci) => (cat.products||[]).forEach((p, pi) => {
    ...
    rows.push(`<div style="display:flex;align-items:center;gap:10px;padding:8px;border:1px solid #e2e8f0;border-radius:10px;margin-bottom:6px">
      ...
      <div style="flex:1;font-size:13px;font-weight:600">${escH(p.name)}</div>
      <button onclick="aiGenProdImg('${tid}',${ci},${pi})" style="padding:6px 12px;border:1px solid #6366f1;border-radius:8px;background:#fff;color:#6366f1;cursor:pointer;font-size:12px" id="aib-img-btn-${tid}">🪄 צור</button>
    </div>`);
  }));
  el.innerHTML = rows.join('') + (rows.length > 1 ? `<button onclick="aiGenAllImgs()" style="margin-top:8px;padding:10px 20px;background:#6366f1;color:#fff;border:none;border-radius:10px;cursor:pointer;font-size:13px;font-weight:600;width:100%">🪄 צור תמונות לכולם</button>` : '');
...
async function aiGenProdImg(tid, ci, pi) {
  ...
  } catch(e) { showToast('error', 'שגיאה בייצור תמונה'); }
  if (btn) { btn.disabled = false; btn.textContent = '🪄 צור'; }
}
...
async function aiCreateBusiness() {
  ...
  try {
    ...
    const r = await fetch('/api/sa/ai-create-business', {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({ generatedData: _aiBuilderData, storeType, images: _aiBuilderImages })
    });
    const d = await r.json();
    if (!d.success) throw new Error(d.error || 'שגיאה ביצירה');
    document.getElementById('aib-progress').style.display = 'none';
...
async function aiSaveTemplate() {
  ...
  const d = await r.json();
  if (d.success) showToast('success', 'Template נשמר!');
...
async function aiLoadTemplates() {
  ...
  const el = document.getElementById('aib-templates-list');
  if (!el) return;
  el.innerHTML = d.templates.length
    ? d.templates.map(t => `<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:6px">
        <div style="flex:1"><div style="font-weight:600;font-size:13px">${escH(t.name)}</div><div style="font-size:11px;color:#94a3b8">${t.business_type||''} · ${new Date(t.created_at).toLocaleDateString('he-IL')}</div></div>
        <button onclick="aiLoadTemplate(${t.id})" style="padding:5px 12px;border:1px solid #6366f1;border-radius:6px;background:#fff;color:#6366f1;cursor:pointer;font-size:12px">טען</button>
        <button onclick="aiDeleteTemplate(${t.id},this)" style="padding:5px 10px;border:1px solid #fca5a5;border-radius:6px;background:#fff;color:#ef4444;cursor:pointer;font-size:12px">🗑️</button>
      </div>`).join('')
    : '<div style="color:#94a3b8;font-size:13px;padding:12px">אין templates שמורים</div>';
...
async function aiLoadTemplate(id) {
  const r = await fetch(`/api/sa/business-templates/${id}`);
  const d = await r.json();
  if (!d.success) return showToast('error', 'לא נמצא');
  _aiBuilderData = d.template.json_data;
  _aiBuilderImages = { products: {} };
  document.getElementById('aib-step1').style.display = 'none';
  document.getElementById('aib-preview').style.display = 'block';
  document.getElementById('aib-templates-panel').style.display = 'none';
  renderAIBPreview();
}
...
async function aiDeleteTemplate(id, btn) {
  ...
  const r = await fetch(`/api/sa/business-templates/${id}`, { method: 'DELETE' });
  const d = await r.json();
  if (d.success) { btn.closest('div').remove(); showToast('success', 'נמחק'); }
}

function switchAIBTab(tab) {
  ['profile','catalog','promotions','images'].forEach(t => {
    document.getElementById(`aib-tab-${t}`)?.classList.toggle('aib-tab-active', t === tab);
    const panel = document.getElementById(`aib-panel-${t}`);
    if (panel) panel.style.display = t === tab ? 'block' : 'none';
  });
}
...
function aiDeletePromo(i) { _aiBuilderData.promotions.splice(i,1); renderAIBPromos(); }
function aiAddCat() { _aiBuilderData.catalog.push({category:'קטגוריה חדשה',category_en:'New Category',products:[]}); renderAIBCatalog(); }
function aiAddProd(ci) { _aiBuilderData.catalog[ci].products.push({name:'מוצר חדש',name_en:'New Product',description:'',description_en:'',price:30,badge_text:'',options_text:''}); renderAIBCatalog(); renderAIBImages(); }
function aiAddPromo() { _aiBuilderData.promotions.push({title:'מבצע חדש',title_en:'New Promo',discount_pct:10,min_order:0}); renderAIBPromos(); }
function aiResetBuilder() {
  _aiBuilderData = null;
  _aiBuilderImages = { products: {} };
  const step1 = document.getElementById('aib-step1');
  const preview = document.getElementById('aib-preview');
  const success = document.getElementById('aib-success');
  const progress = document.getElementById('aib-progress');
  const generateBtn = document.getElementById('aib-generate-btn');
  if (step1) step1.style.display = 'block';
  if (preview) preview.style.display = 'none';
  if (success) success.style.display = 'none';
  if (progress) progress.style.display = 'none';
  if (generateBtn) { generateBtn.disabled = false; generateBtn.textContent = '✨ צור עסק עם AI'; }
}
